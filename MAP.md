...
- **Phase 12 (IL2CPP managed layer): ⏳ items 0–2 + live discovery.** `il2cpp import` / `il2cpp symbols` import an external dump (Il2CppDumper `script.json`) as a named index under `.n0x/il2cpp/`, with the address convention *measured* against the target rather than assumed, and Unity WebGL indices structurally barred from binding to a native image. `il2cpp metadata` reads `global-metadata.dat` natively (no external dumper for the version-independent half); `il2cpp icalls` / `classes` / `obj` discover engine natives and the runtime type system on a live process; and the imported index now chains through the `SymbolProvider` seam so `decomp pseudo` / `xref` / `function trace` render managed names automatically. ⬜ Devirtualization (item 3) is still open.

The command count is auto-generated by `n0x guide` from the clap tree, so it never drifts; rebuild to see the current total (the network/`net` layer has been split out into a separate plugin repository and is no longer part of this tree).

Of those 91, **45 are backed by the capability registry** (`n0xis-frontend::registry`), where the CLI and MCP handlers are argument mapping over one shared implementation; the rest still carry a per-frontend handler each. See the ROADMAP's "Engineering hardening" section for the migration state.
