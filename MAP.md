...
| `n0xis-il2cpp` | IL2CPP managed layer (Phase 12): imported managed symbol indices served through `SymbolProvider`, plus a native `global-metadata.dat` reader and live runtime-type discovery (`il2cpp metadata` / `icalls` / `classes` / `obj`). Address spaces are explicit, so an IL2CPP WebGL dump can never bind to a native PE. | **No** |

One layer is deliberately **not** in this table: the WARP interoperability
layer (structural function-identity GUIDs and a `.warp` reader) is a plugin in a
separate repository. It is not built here, and `crates/n0xis-warp/` is
git-ignored so a local checkout cannot re-enter this repo. Nothing in this build
reads a `.warp` file.

### Status at a glance (verify against [[ROADMAP]] before quoting)
- **Phases 1–7: done.** Phase 3 optimizing SSA decompiler; 4b live memory (value/pointer/AOB scanning, freeze, patching); 4c provenance; 5 MCP; 6 persistence/caching/snapshot/remote/perf; 7 ARM64 + value-set + deobfuscation + diffing.
- **ARM64 (measured 2026-09-07):** the **decoder and CFG are verified to x64's standard** on a real AArch64 library — 2 381/2 381 function extents exactly equal to the symbol table, 0 invariant violations across 46 757 CFG edges. ⚠️ The **decompiler is not**: the AArch64 lift/SSA is not built, so `decomp pseudo` reports `quality: 0.0` and falls back to `asm:` nodes. Say "decoder and CFG verified, decompiler x64-only" — never "ARM64 works."
...
- **Phase 11 (agent consumability): done and committed.**
- **Phase 12 (IL2CPP managed layer): ⏳ items 0–2 + live discovery.** `il2cpp import` / `il2cpp symbols` import an external dump (Il2CppDumper `script.json`) as a named index under `.n0x/il2cpp/`, with the address convention *measured* against the target rather than assumed, and IL2CPP WebGL indices structurally barred from binding to a native image. `il2cpp metadata` reads `global-metadata.dat` natively (no external dumper for the version-independent half); `il2cpp icalls` / `classes` / `obj` discover engine natives and the runtime type system on a live process; and the imported index now chains through the `SymbolProvider` seam so `decomp pseudo` / `xref` / `function trace` render managed names automatically. ⬜ Devirtualization (item 3) is still open.

The command count is auto-generated by `n0x guide` from the clap tree, so it never drifts; rebuild to see the current total (the WARP interoperability layer has been split out into a separate plugin repository and is no longer part of this tree). It currently reports **113 leaf commands**, and `crates/n0xis-cli/tests/docs_match_binary.rs` fails the build if a document says otherwise.

Of the 62 capabilities registered, **60 are dispatched by the CLI**; `decode` and `function.discover` are reached only from MCP and `capability run`, and the CLI keeps its own handler for the same work. Counted from the names `main.rs` passes to `run_capability` — including the three (`ir.cfg`, `ir.dot`, `ir.explain`) it passes through a variable, which a first count of this missed and reported as unreachable. See the ROADMAP's "Engineering hardening" section for the migration state.
...
The Phase 3 optimizing SSA decompiler is the largest subsystem here. Build IR, decompile to pseudo-C, slice, xref, discover/trace functions, diff.
