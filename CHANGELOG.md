...
- **2a — escape analysis.** A stack slot whose address never becomes a value
  ...
  and the Win64 shadow space. Cross-block forwarding on the compression DLL
...
- **Into the decompiler.** A vtable constant renders `&Class::vtable`; a
  ...
  std::exception`; the 561-vtable build's 5-level ICU chains). Verified against known STL
  ground truth.
...
- **5g — immediate rotate lift** (`rol`/`ror` → shift/shift/or, laying hash
  ...
  `mulx`/`cdqe`/`btr`…, from a PE/MSVC shipping build's newer-ISA finding).
...
- **Rung 5a — branch conditions from arithmetic flags.** A `Jcc` after a
  ...
  Loop latches get a visible condition again — on the compression DLL,
...
- **Rung 4a — precise register-argument arity (Win64).** A register counts
  ...
  function at `(rcx, rdx, r8, r9)`. On the compression DLL the arity-4 rate
...
- **Rung 3a — parameter typing from use.** A parameter's signature type is
  ...
  the compression DLL recover a pointer/struct parameter type.
