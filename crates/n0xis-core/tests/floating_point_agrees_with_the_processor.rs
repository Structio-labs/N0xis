...
//! **The processor is the oracle, for the floating point too.**
//!
//! The census of a shipped library ranks what stops the emulator, and after
//! calls were modelled the largest remaining block is vector and
//! floating-point: about 90 of 400 functions. Nothing had checked that layer —
//! not wrongly, *not at all*. The lift renders every FP instruction as a named
//! intrinsic (`__addsd(x, y)`), which is an honest way to say "this is what the
//! instruction was" and no way at all to say "this is what it computes".
//!
//! An absence of complaints is not evidence. So: `oracle/emu_fp.c`, the same
//! driver, the same four optimization levels, and the same rule — the recovered
//! function is handed the arguments the processor was handed, and the numbers
//! must match.
//!
//! The inputs are what make this worth running. The driver's value list read as
//! doubles is NaN, negative zero, the smallest denormal, and two ordinary
//! numbers, and those are exactly where a plausible model differs from the
//! hardware: `minsd` is not `fmin` (NaN returns the *second* source), `jb`
...
#[test]
fn floating_point_computes_what_the_processor_computes() {
    if !have("gcc") || !have("nm") {
        eprintln!("skip: needs gcc and nm — nothing was checked");
        return;
    }
    let dir = oracle_dir();
    let tmp = std::env::temp_dir().join("n0xis-emu-fp");
    ...
    for cc in compilers() {
        for level in ["-O0", "-O1", "-O2", "-Os"] {
            let so = tmp.join(format!("emu_fp-{cc}{level}.so"));
            ...
            let built = Command::new(cc)
                ...
                .arg(&so)
                .arg(dir.join("emu_fp.c"))
                .status()
                .expect("the compiler runs");
            assert!(built.success(), "{cc} could not build oracle/emu_fp.c at {level}");
