...
//! **Packed SIMD, against the processor.**
//!
//! The scalar floating-point layer is measured; this is the other half, and the
//! largest single block the census of a shipped library names. One instruction
//! operates on four lanes at once, and taking the low one is not a smaller
//! answer — it is a different value in every bit above the first lane.
//!
//! Nothing here could even be *represented* until the emulator's word grew to
//! 128 bits. That widening is checked by everything else in this directory
//! continuing to agree: scalar arithmetic still narrows to 64 bits at every
//! `Binary` but the bitwise ones, so seventeen thousand existing comparisons
//! are the control group for it.
//!
...
#[test]
fn packed_operations_compute_what_the_processor_computes() {
    if !have("gcc") || !have("nm") {
        eprintln!("skip: needs gcc and nm — nothing was checked");
        return;
    }
    let dir = oracle_dir();
    let tmp = std::env::temp_dir().join("n0xis-emu-simd");
    ...
    for cc in compilers() {
        for level in ["-O0", "-O1", "-O2", "-Os"] {
            let so = tmp.join(format!("emu_simd-{cc}{level}.so"));
            ...
            let built = Command::new(cc)
                ...
                .arg(&so)
                .arg(dir.join("emu_simd.c"))
                .status()
                .expect("the compiler runs");
            assert!(built.success(), "{cc} could not build oracle/emu_simd.c at {level}");
