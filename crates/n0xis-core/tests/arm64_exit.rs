...
fn cfg_pass_builds_a_correct_graph_over_real_arm64_bytes() {
    ...
    // decoded instruction reports reading the tested register, proving
    // Arm64::reg_access is wired into the same def-use path X64 uses.
    //
    // The name is **`x0`, not `w0`**, even though these bytes encode the 32-bit
    // view. One physical register gets one name in the records, exactly as x86
    // records `rax` for a write to `eax` — because when it did not, `ir slice
    // --reg x0` answered `node_count: 0` on a function whose only instruction
    // is `add w0, w0, w1`. The disassembly text still reads `w0`; only the
    // records are canonical.
    let cbz_insn = &cbz_block.insns[0];
    assert_eq!(cbz_insn.reads, vec!["x0".to_string()]);
    ...
    assert_eq!(add_insn.writes, vec!["x0".to_string()]);
    assert_eq!(add_insn.reads, vec!["x0".to_string(), "x0".to_string()]);
