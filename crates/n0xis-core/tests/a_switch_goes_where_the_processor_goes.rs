...
//! **Computed control flow, checked against the processor.**
//!
//! A jump table resolved wrongly is the worst answer a decompiler can give:
//! not a number that is off by something, but *control flow that never
//! happened*, and every fact derived downstream inherits it. The census of a
//! shipped library shows 19 of 400 functions stopping at an `ijmp`, and until
//! now nothing anywhere had checked that the cases n0xis recovers are the cases
//! the machine takes. The switch resolver's own tests compare it against
//! itself, which is a contract check wearing the clothes of a correctness one.
//!
//! Two things are measured here at once, and the second is the point:
//!
//! * the recovered function **returns what the hardware returned**, which is
//!   the ordinary comparison every corpus in this directory makes;
//! * every indirect jump lands on an address the CFG **already recorded as an
//!   edge**. The emulator computes the destination from the IR — the lift now
//!   carries it, where it used to be dropped as "structural" — and refuses to
//!   follow an edge the resolver never claimed. A case the resolver missed, or
//!   a table whose bounds it read wrongly, comes out as
//!   `EmuError::UnrecordedJump` naming the address.
//!
...
#[test]
fn a_switch_dispatches_where_the_processor_dispatches() {
    if !have("gcc") || !have("nm") {
        eprintln!("skip: needs gcc and nm — nothing was checked");
        return;
    }
    let dir = oracle_dir();
    let tmp = std::env::temp_dir().join("n0xis-emu-switch");
    ...
    for cc in compilers() {
        for level in ["-O0", "-O1", "-O2", "-Os"] {
            let so = tmp.join(format!("emu_switch-{cc}{level}.so"));
            ...
            let built = Command::new(cc)
                ...
                .arg(&so)
                .arg(dir.join("emu_switch.c"))
                .status()
                .expect("the compiler runs");
            assert!(built.success(), "{cc} could not build oracle/emu_switch.c at {level}");
