...
fn extract(ctx: &Ctx, va: Va, max_bytes: usize, known: &BTreeSet<u64>) -> Option<FnFacts> {
    ...
    let types = TypeInferPass.run(ctx, TypeInferInput { cfg: cfg.clone(), blocks: opt.blocks.clone(), float_return: opt.float_return, float_return_pair: opt.float_return_pair }).ok()?;
    ...
    let seed_ret = types.signature.ret.ctype().and_then(|t| t.name.clone());
    ...
    // A call reaches this pass in **two** shapes, and reading only one of them
    // dropped most of the call graph. The optimizer folds a call whose result
    // has a single consumer into that consumer, removing the `Call` *statement*
    // and leaving a `Call` *expression* in its place — so `return f(x) + 1`
    // carries its edge inside the return expression. Measured on a target built
    // for this: three calls in the image, one edge seen, nothing propagated.
    let push_call = |target: &CallTarget, args: &[MicroExpr], ret: Option<String>, calls: &mut Vec<CallFact>| {
        let CallTarget::Direct { va: callee } = target else { return };
        // A shared object calls its **own** functions through the PLT, because
        // the linker routes them that way so `LD_PRELOAD` can interpose. The
        // literal target is then a stub, not a body, and keying on it loses the
        // edge entirely: measured at zero edges between the corpus functions of
        // a C++ shared library, which is why this pass propagated nothing there.
        let callee = ctx.symbols.and_then(|s| s.thunk_to(*callee)).unwrap_or(*callee);
        // Only calls *inside the analyzed program* are edges; an import has a
        // signature library, not a recovered one.
        if !known.contains(&callee.0) || callee.0 == va.0 {
            return;
        }
        calls.push(CallFact { callee: callee.0, args: args.iter().map(as_var).collect(), ret });
    };
        ...
        for stmt in &block.stmts {
            match &stmt.stmt {
                MicroStmt::Assign { dst, value } => {
                    ...
                    for (t, a) in calls_in(value) {
                        push_call(t, a, None, &mut calls);
                    }
                }
                MicroStmt::Call { target, args, ret } => {
                    push_call(target, args, ret.clone(), &mut calls);
                    for arg in args {
                        for (t, a) in calls_in(arg) {
                            push_call(t, a, None, &mut calls);
                        }
                    }
                }
                MicroStmt::Return(Some(e)) => {
                    ret_var = as_var(e).or(ret_var.take());
                    for (t, a) in calls_in(e) {
                        push_call(t, a, None, &mut calls);
                    }
                }
...
/// Every call **inside an expression** — the shape the optimizer leaves behind
/// when it inlines a call result into its only consumer. A `Call` statement and
/// a `Call` expression are the same edge in the program's call graph, and a pass
/// that reads one and not the other sees a graph that is not the program's.
fn calls_in(e: &MicroExpr) -> Vec<(&CallTarget, &[MicroExpr])> {
    let mut out = Vec::new();
    collect_calls(e, &mut out);
    out
}

fn collect_calls<'a>(e: &'a MicroExpr, out: &mut Vec<(&'a CallTarget, &'a [MicroExpr])>) {
    match e {
        MicroExpr::Call { target, args } => {
            out.push((target, args.as_slice()));
            for a in args {
                collect_calls(a, out);
            }
        }
        MicroExpr::Unary(_, i) | MicroExpr::Cast { expr: i, .. } | MicroExpr::AddrOf(i) => {
            collect_calls(i, out)
        }
        MicroExpr::Load { addr, .. } => collect_calls(addr, out),
