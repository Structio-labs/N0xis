...
pub fn slice(arch: &dyn Arch, art: &CfgArtifact, at: Va, reg: &str) -> SliceArtifact {
    ...
    // The stack pointer is plumbing, not data. Every `call` reads it, so
    // following that edge pulls the whole prologue into the answer to a
    // question about a value: slicing the result of `middle(v) + leaf_a(v)`
    // returned `push rsi`, `push rbx` and `sub rsp,28h` alongside the three
    // instructions that actually compute it. Followed only when the slice is
    // *about* the stack pointer.
    let sp = arch.stack_pointer().map(|s| arch.normalize_reg(s));
    let slicing_the_stack_pointer = sp.as_deref() == Some(reg.as_str());
    let is_plumbing = |edge_reg: &str| !slicing_the_stack_pointer && sp.as_deref() == Some(edge_reg);
...
mod tests {
    ...
    /// The register the disassembly prints is the register you can ask about.
    ///
    /// The def-use records a vector write under its widest name (`zmm0`) while
    /// every line of output says `xmm0`. Nothing bridged the two, so asking for
    /// a slice of `xmm0` on a function whose only instruction is
    /// `addsd xmm0,xmm1` answered `node_count: 0` — an empty answer with
    /// `ok: true`, which is the worst shape a wrong answer can take.
    #[test]
    fn a_vector_register_is_sliceable_under_the_name_the_disassembly_prints() {
        let arch = X64::new();
        let art = build(vec![0xF2, 0x0F, 0x58, 0xC1, 0xC3]);
        for name in ["xmm0", "ymm0", "zmm0", "XMM0"] {
            let sl = slice(&arch, &art, Va(0x1004), name);
            assert_eq!(sl.nodes.len(), 1, "asking for {name} must find the addsd, got {:?}", sl.nodes);
            assert_eq!(sl.nodes[0].text, "addsd xmm0,xmm1");
        }
    }

    /// A `call` architecturally writes only `rsp`, so with no ABI knowledge
    /// the definition of the value it produced does not exist and a backward
    /// slice of that value stops at the `mov` that copied it out of the return
    /// register. Measured on a function whose source is `middle(v) +
    /// leaf_a(v)`: the answer to "what computes this" named the three register
    /// moves and neither call.
    #[test]
    fn a_call_is_what_computes_the_value_it_returned() {
        // 0x1000 call +0x10       e8 10 00 00 00   -> 0x1015
        // 0x1005 mov  esi, eax    89 c6
        // 0x1007 call +0x0e       e8 0e 00 00 00   -> 0x101a
        // 0x100c add  esi, eax    01 c6
        // 0x100e mov  eax, esi    89 f0
        // 0x1010 ret             c3
        let code = vec![
            0xe8, 0x10, 0x00, 0x00, 0x00, 0x89, 0xc6, 0xe8, 0x0e, 0x00, 0x00, 0x00, 0x01, 0xc6, 0x89, 0xf0, 0xc3,
        ];
        let art = build(code);
        let arch = X64::new();
        let sl = slice(&arch, &art, Va(0x100e), "rax");
        let addrs: Vec<u64> = sl.nodes.iter().map(|n| n.va.0).collect();
        assert_eq!(
            addrs,
            vec![0x1000, 0x1005, 0x1007, 0x100c, 0x100e],
            "both calls produce a value the result depends on and must be in the slice"
        );
    }

    /// The stack pointer is plumbing. Every call reads it, so following that
    /// edge answers a question about a computed value with the prologue —
    /// three stack instructions appeared beside the three that compute the
    /// value. It is followed only when the slice is *about* the stack pointer.
    #[test]
    fn the_stack_pointer_is_not_part_of_a_data_slice() {
        // 0x1000 push rbx        53
        // 0x1001 sub  rsp, 0x28  48 83 ec 28
        // 0x1005 call +0x06      e8 06 00 00 00
        // 0x100a mov  eax, eax   89 c0
        // 0x100c ret             c3
        let code = vec![0x53, 0x48, 0x83, 0xec, 0x28, 0xe8, 0x06, 0x00, 0x00, 0x00, 0x89, 0xc0, 0xc3];
        let art = build(code);
        let arch = X64::new();
        let sl = slice(&arch, &art, Va(0x100a), "rax");
        let addrs: Vec<u64> = sl.nodes.iter().map(|n| n.va.0).collect();
        assert_eq!(addrs, vec![0x1005, 0x100a], "the call and the move, not the frame setup");

        // …and asking about the stack pointer itself still answers.
        let sp = slice(&arch, &art, Va(0x1005), "rsp");
        assert!(
            sp.nodes.iter().any(|n| n.va.0 == 0x1001),
            "a slice of rsp must still reach `sub rsp,0x28`: {:?}",
            sp.nodes.iter().map(|n| n.va.0).collect::<Vec<_>>()
        );
    }
