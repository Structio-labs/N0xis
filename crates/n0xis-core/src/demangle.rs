...
fn demangle_itanium(name: &str) -> Option<String> {
    ...
    sym.demangle().ok().map(|d| gnu_special_name(&d))
}

/// Render the Itanium ABI's *special names* the way the C++ world writes them.
///
/// The demangler used here has its own brace syntax for them —
/// `{vtable(QImage)}` where every other tool writes `vtable for QImage`, and
/// `{virtual override thunk({offset(-16)}, f())}` for all three thunk kinds at
/// once. It renders `typeinfo for` and `guard variable for` conventionally, so
/// the output was inconsistent with itself as well as with everything else.
///
/// Measured on one Qt build: **305 of 8 349** mangled symbols came out in a
/// brace form — 269 vtables and 36 thunks. They are exactly the symbols that
/// name C++ dispatch, so anything matching the conventional spelling failed on
/// precisely the names it cared about.
///
/// A shape this does not recognise is returned untouched: a wrong translation
/// would be worse than an unfamiliar one.
fn gnu_special_name(d: &str) -> String {
    let Some(inner) = d.strip_prefix('{').and_then(|r| r.strip_suffix('}')) else {
        return d.to_string();
    };
    if let Some(x) = paren_arg(inner, "vtable") {
        return format!("vtable for {x}");
    }
    if let Some(x) = paren_arg(inner, "vtt") {
        return format!("VTT for {x}");
    }
    if let Some(args) = paren_arg(inner, "virtual override thunk") {
        // The demangler spells all three thunk kinds the same way and tells
        // them apart only by the adjustment groups: one plain offset is a
        // non-virtual thunk, one virtual offset is a virtual thunk, and two
        // groups are a covariant return thunk.
        let parts = split_top(&args);
        let Some(target) = parts.last() else { return d.to_string() };
        let kind = match parts.len() {
            2 if parts[0].starts_with("{virtual offset") => "virtual thunk to",
            2 => "non-virtual thunk to",
            3 => "covariant return thunk to",
...
/// The argument text of `head(...)`, when `s` is exactly that.
fn paren_arg(s: &str, head: &str) -> Option<String> {
    let rest = s.strip_prefix(head)?.strip_prefix('(')?.strip_suffix(')')?;
    Some(rest.to_string())
}

/// Split on top-level commas — the adjustment groups carry their own.
...
mod tests {
    ...
    /// Every one of these is what `c++filt` prints for the same symbol. The
    /// demangler this wraps spells the special names in its own brace syntax,
    /// which is readable but matches nothing else — including the conventional
    /// `typeinfo for` it already emitted, so its output disagreed with itself.
    #[test]
    fn the_itanium_special_names_read_the_way_the_c_plus_plus_world_writes_them() {
        for (mangled, want) in [
            ("_ZTV6QImage", "vtable for QImage"),
            ("_ZTVN10__cxxabiv117__class_type_infoE", "vtable for __cxxabiv1::__class_type_info"),
            ("_ZTT6QImage", "VTT for QImage"),
            ("_ZTI6QImage", "typeinfo for QImage"),
            ("_ZTS6QImage", "typeinfo name for QImage"),
            ("_ZThn16_N6QImage3fooEv", "non-virtual thunk to QImage::foo()"),
            ("_ZTv0_n24_N6QImage3fooEv", "virtual thunk to QImage::foo()"),
            ("_ZTch0_h16_N6QImage3fooEv", "covariant return thunk to QImage::foo()"),
            ("_ZTcv0_n24_v0_n32_N6QImage3fooEv", "covariant return thunk to QImage::foo()"),
            ("_ZGVZN6QImage4dataEvE1x", "guard variable for QImage::data()::x"),
            ("_ZN6QImage6formatEv", "QImage::format()"),
        ] {
            assert_eq!(demangle(mangled), want, "{mangled}");
        }
    }

    #[test]
    fn a_brace_form_this_does_not_recognise_is_left_alone() {
        // A wrong translation is worse than an unfamiliar one.
        assert_eq!(gnu_special_name("{something new(x)}"), "{something new(x)}");
        assert_eq!(gnu_special_name("{virtual override thunk(a, b, c, d)}"), "{virtual override thunk(a, b, c, d)}");
        assert_eq!(gnu_special_name("QImage::format()"), "QImage::format()");
    }
    use super::*;
