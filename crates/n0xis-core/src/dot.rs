...
mod tests {
    ...
    fn switch_cases_appear_as_edges() {
        // The same mem-indexed switch the `ir` test builds: a guard, the
        // dispatch, and three case bodies **inside** the function, which is
        // where a compiler puts them. The graph must carry an edge to each.
        //
        // This test used to place the cases outside the decoded function and
        // assert they rendered as *external* edges. On real code that shape
        // does not occur — measured on 1 812 Qt functions, every switch edge
        // leaving the function came from a table read past its end — so the
        // fixture was asserting the behaviour that produced invented control
        // flow.
        let mut code = vec![
            0x48, 0x83, 0xf8, 0x02, // cmp rax, 2
            0x77, 0x28, //             ja +0x28 → the default
        ...
        let d = dot(&art);
        assert_eq!(d.external_count, 0, "every case is a block of this function");
        assert!(d.dot.contains("switch-case"), "the edges are labelled");
        for case in ["0x1010", "0x1018", "0x1020"] {
