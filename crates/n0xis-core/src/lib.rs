...
pub use classlayout::{
    ClassLayout, ClassLayoutInput, ClassLayoutPass, FieldObs, LayoutStore, this_class_of,
};
...
pub use emulate::{
    BodyMap, CALLER_FRAME, CallPolicy, CodeProvider, DEFAULT_FS_BASE, DEFAULT_GS_BASE,
    DEFAULT_STACK_TOP, EmuConfig, EmuError, EmuResult, Emulator, RETURN_SENTINEL,
};
...
pub use typeinfer::{
    CType, FieldAccess, LocalVar, ParamInfo, RecoveredSignature, RecoveredType, ReturnType,
    TypeArtifact, TypeInferInput, TypeInferPass, abi_arg_registers,
};
...
pub struct Ctx<'a> {
    ...
    /// Classes with at least one **subclass in this image**.
    ///
    /// A virtual dispatch resolved through the *declared type of a field* is
    /// sound as a type and unsound as a vtable — the object stored there may be
    /// a derived class whose slot holds a different method. With this set,
    /// [`devirtualize`] refuses exactly those and keeps every other basis.
    /// `None` = no hierarchy recovered, and the refusal cannot be made.
    pub subclassed: Option<&'a std::sync::Arc<std::collections::HashSet<String>>>,
    ...
    /// Function starts and exclusive ends the **image itself states** — ELF
    /// `.eh_frame` FDEs, a PE `.pdata` table ([`scan_eh_frame`]/[`scan_pdata`]).
    ///
    /// Discovery is a prologue *scan*, and a byte pattern is not evidence of a
    /// function: on one Qt build it invented 4 483 candidates an independent
    /// function table calls the middle of other functions, and missed 1 533
    /// starts the binary declares outright. A stated start is a fact and goes
    /// in first; the scan then fills the gaps it leaves. `None` = the scan
    /// alone, exactly as before.
    ///
    /// **Sorted by start address**, which every consumer relies on to look a
    /// function up rather than walk the list once per query.
    pub functions: Option<&'a [(n0xis_contracts::Va, n0xis_contracts::Va)]>,
...
impl<'a> Ctx<'a> {
    ...
    /// Attach the set of classes that have a subclass here (see the field docs).
    pub fn with_subclassed(mut self, s: &'a std::sync::Arc<std::collections::HashSet<String>>) -> Self {
        self.subclassed = Some(s);
        self
    }
    ...
    /// Attach the function starts/ends the image states (see the field docs).
    pub fn with_stated_functions(mut self, f: &'a [(n0xis_contracts::Va, n0xis_contracts::Va)]) -> Self {
        self.functions = Some(f);
        self
    }
