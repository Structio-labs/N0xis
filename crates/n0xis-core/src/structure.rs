...
struct Ctx<'a> {
    ...
    budget: usize,
    /// Blocks whose `// block_N:` anchor must survive, because a `goto` in the
    /// same function targets them. Empty on the first rendering pass — see
    /// [`structure`], which learns the set from that pass and renders again.
    keep_labels: &'a BTreeSet<usize>,
}
...
fn emit_if_else(ctx: &mut Ctx, n: usize, until: Option<usize>) {
    ...
    // The short-circuit fold collapses a chain of comparison blocks into one
    // condition (`if (a && b)`) and marks the swallowed blocks visited without
    // emitting them — so their `// block_N:` anchors never exist. Right until a
    // `goto` elsewhere targets one: then the fold silently removes the only
    // label in the function while the jump to it stays. `keep_labels` names
    // exactly those blocks (learned from the previous rendering pass), and a
    // chain containing one is not folded.
    if let Some((folded_cond, body_arm, else_arm, eaten)) = try_short_circuit(ctx, n, &cond, t, f)
        .filter(|(_, _, _, eaten)| !eaten.iter().any(|b| ctx.keep_labels.contains(b)))
    {
        for b in &eaten {
            ctx.visited[*b] = true;
        }
        // Buffer both arms, exactly as the unfolded path below does, so an
        // empty arm gets the same cleanup. Emitting them straight into the
        // output made this the one shape that escaped it: a folded `||`
        // whose body sits in the *else* rendered as `if (a || b) { } else
        // { … }`. Measured against a second decompiler over 200 functions of
        // one C++ runtime — 20 of them here, none there.
        ctx.indent += 1;
        let body_lines = capture_arm(ctx, body_arm, merge);
        let else_lines = if Some(else_arm) != merge { capture_arm(ctx, else_arm, merge) } else { Vec::new() };
        ctx.indent -= 1;
        let p0 = pad(ctx.indent);
        let body_has = arm_has_code(&body_lines, ctx.keep_labels);
        let else_has = arm_has_code(&else_lines, ctx.keep_labels);
        if !body_has && !else_has {
            // Same as the unfolded path: a decision with nothing on either side.
            if let Some(m) = merge {
        ...
        if !body_has && else_has {
            // The negation of a folded condition is not a field this holds, so
            // it is spelled rather than re-derived: `!(a || b)`.
            ctx.out.push(format!("{p0}if (!({folded_cond})) {{"));
            ctx.out.extend(else_lines);
        } else {
            ctx.out.push(format!("{p0}if ({folded_cond}) {{"));
            ctx.out.extend(body_lines);
            if else_has {
                ctx.out.push(format!("{p0}}} else {{"));
                ctx.out.extend(else_lines);
            }
        }
        ctx.out.push(format!("{p0}}}"));
        if let Some(m) = merge {
            emit_node(ctx, m, until);
        }
        return;
    ...
    // An arm with no code line is dropped whole — right for an arm that renders
    // to nothing, wrong when it carries the label something jumps to. A block
    // whose statements are all noise (a bare `cmp`, an epilogue's
    // `add rsp,N; jmp rax`) renders to its anchor and nothing else, and
    // discarding the arm discarded the only `block_N:` in the function while
    // the `goto` to it stayed. There the label *is* the content.
    let pad0 = pad(ctx.indent);
    // Neither arm carries anything: the decision has no consequence in either
    // direction, and printing it claims a distinction the text does not make.
    // Both paths reach the merge, which is emitted below either way, so the
    // `if` is dropped whole rather than rendered as `if (c) { } else { }`.
    // (An arm that only holds a needed `block_N:` anchor counts as carrying
    // something — `arm_has_code` says so — and keeps the branch.)
    if !arm_has_code(&then_lines, ctx.keep_labels) && !arm_has_code(&else_lines, ctx.keep_labels) {
        if both_diverge {
            return;
        }
        if let Some(m) = merge {
    ...
    if !arm_has_code(&then_lines, ctx.keep_labels) && arm_has_code(&else_lines, ctx.keep_labels) {
    ...
    } else if arm_has_code(&then_lines, ctx.keep_labels) && !arm_has_code(&else_lines, ctx.keep_labels) {
        ...
        if let Some(collapsed) = try_else_if(&else_lines, ctx.indent, ctx.keep_labels) {
...
/// Does a captured arm carry anything the reader needs? A statement, or the
/// `block_N:` anchor some `goto` targets — for the latter the label *is* the
/// content, and dropping the arm would strip the only definition of a label the
/// jump to it still names.
fn arm_has_code(ls: &[String], keep: &std::collections::BTreeSet<usize>) -> bool {
    ls.iter().any(|l| is_code_line(l))
        || ls.iter().filter_map(|l| anchor_block_id(l)).any(|n| keep.contains(&n))
}
...
/// The block id of a rendered `// block_N: …` anchor line, if that is what it is.
fn anchor_block_id(l: &str) -> Option<usize> {
    l.trim_start().strip_prefix("// block_")?.split(':').next()?.trim().parse().ok()
}
...
fn try_else_if(else_lines: &[String], indent: usize, keep_labels: &BTreeSet<usize>) -> Option<Vec<String>> {
    let first_code = else_lines.iter().position(|l| is_code_line(l))?;
    // The collapse drops this block's own `// block_N:` anchor, which is fine
    // until something jumps to it: 53 `goto block_N;` on one Qt build had no
    // label anywhere in the text because the block they targeted had been
    // folded into an `else if`. A label cannot go between `} else` and the
    // condition, so the block simply does not collapse when it is a target.
    if else_lines[..first_code].iter().filter_map(|l| anchor_block_id(l)).any(|n| keep_labels.contains(&n)) {
        return None;
    }
...
pub fn structure(cfg: &CfgArtifact, blocks: &[SsaBlock], names: &RenderNames) -> StructuredOutput {
    // Two passes, because whether a block needs a label is not known until the
    // whole function has been laid out: the `else if` collapse drops a block's
    // anchor, and only afterwards is it clear that something jumps there. The
    // first pass answers that question, the second acts on it. A function with
    // no such conflict renders once — the set comes back empty and the second
    // pass is skipped.
    // Iterated, because keeping one label changes the layout and can expose
    // another block in the same position. Bounded: each round can only *add*
    // to the set, so it either reaches a fixpoint or stops, and stopping leaves
    // the best rendering found rather than looping.
    const ROUNDS: usize = 4;
    let mut keep = BTreeSet::new();
    let mut out = structure_once(cfg, blocks, names, &keep);
    for _ in 0..ROUNDS {
        let missing = gotos_without_labels(&out.lines);
        if missing.is_empty() || missing.is_subset(&keep) {
            break;
        }
        keep.extend(missing);
        out = structure_once(cfg, blocks, names, &keep);
    }
    out
}

/// Block ids a `goto` targets that have no `// block_N:` anchor left in `lines`.
fn gotos_without_labels(lines: &[String]) -> BTreeSet<usize> {
    let mut labelled = BTreeSet::new();
    let mut wanted = BTreeSet::new();
    for l in lines {
        let t = l.trim_start();
        if let Some(r) = t.strip_prefix("// block_")
            && let Some(n) = r.split(':').next().and_then(|x| x.trim().parse::<usize>().ok())
        {
            labelled.insert(n);
        }
        if let Some(r) = t.strip_prefix("goto block_")
            && let Ok(n) = r.trim_end().trim_end_matches(';').trim().parse::<usize>()
        {
            wanted.insert(n);
        }
    }
    wanted.difference(&labelled).copied().collect()
}
...
fn structure_once(
    ...
    // `real_line_count > 0` skips a block whose statements all render to
    // nothing — a bare `cmp` (its only effect is flags, which the renderer
    // drops as noise) or an epilogue's `add rsp,N; jmp rax`. That is right
    // until a `goto` targets it: then the **label is the content**, and
    // without it the output carried a `goto` to a block it never printed.
    // `keep_labels` names exactly those, learned from the previous pass.
    while let Some(b) = (0..n).find(|&b| {
        !ctx.visited[b] && (real_line_count(&ctx, b) > 0 || ctx.keep_labels.contains(&b))
    }) {
