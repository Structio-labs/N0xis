...
/// 1. A real MSVC member symbol (`?method@Class@@QEAA…`), which encodes
///    non-staticness — [`crate::demangle::member_function_class`].
/// 2. A name of the form `Class::something` whose `Class` is one RTTI recovered
///    a **vtable** for. That is not a guess about naming style: those names are
///    generated by N0xis's own vtable walk, so the function *is* a virtual
...
fn own_this_class(ctx: &Ctx, start: Va) -> Option<String> {
    ...
    let class = bare.rsplit_once("::")?.0;
    if class.is_empty() {
        return None;
    }
    let vtables = ctx.vtables?;
    vtables.values().any(|c| c == class).then(|| class.to_string())
}
...
fn infer_with(ctx: &Ctx, cfg: &CfgArtifact, blocks: &[SsaBlock], deep: bool) -> TypeArtifact {
    ...
    let own_class = own_this_class(ctx, cfg.start);
