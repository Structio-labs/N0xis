...
pub struct OptArtifact {
    ...
    /// The optimizer folds the returned register's *name* out of the expression,
    /// so this is the only thing downstream can still ask "did the result come
    /// back in the float register" — and this pass consumes the SSA artifact, so
    /// carrying it is also what keeps the answer reachable at all.
    ///
    /// In-memory metadata, skipped in the `n0xis.opt.delta.v1` serialization.
    #[serde(skip)]
    pub float_return: bool,
    /// [`SsaArtifact::float_return_pair`](crate::ssa::SsaArtifact::float_return_pair),
...
impl Pass for OptimizePass {
    ...
    fn run(&self, ctx: &Ctx, ssa: SsaArtifact) -> Result<OptArtifact, CoreError> {
        ...
        for _ in 0..MAX_ROUNDS {
            ...
            changed |= expr_prop_round(&mut blocks, &mut delta, ctx.arch.stack_pointer());
            changed |= dce_round(&mut blocks, &mut delta, ctx.arch.stack_pointer());
            if !changed {
                break;
            }
        }
        Ok(OptArtifact {
            ...
            rounds,
            delta,
            float_return: ssa.float_return,
            float_return_pair: ssa.float_return_pair,
        })
    }
}
...
/// Fold a binary operation over two constants — **or refuse**.
///
/// `bits` is the width the caller believes the operation runs at, and it is a
/// belief: `MicroExpr::Binary` carries no width, so `fold_once` takes it from
/// the left operand, whose `bits` is the width of a *literal* and not of the
/// machine operation. `mov eax, 0x1ff; shl rax, 53` is a 64-bit shift whose
/// left operand is a 32-bit immediate; folding it at 32 bits gave **zero**, and
/// zero is a perfectly ordinary-looking constant for the exponent field of a
/// double, so a function that builds a floating-point value that way returned a
/// denormal instead of the number it computed. Nothing downstream could notice:
/// a folded constant carries no trace of what it was.
///
/// So the exact result is computed first, and the fold only happens if `bits`
/// can actually hold it — read either as signed or as unsigned, because
/// `3 - 5` at 32 bits is a legitimate `0xfffffffe`. Anything else is left
/// unfolded, which costs a missed simplification and never states a wrong
/// number. The emulator then computes it at the width the IR really implies,
/// and the processor judges the result.
fn fold_binary(op: BinOp, lhs: i128, rhs: i128, bits: n0xis_arch::Bits) -> Option<i128> {
    ...
    // An arithmetic shift reads its operand as signed; masking first made
    // `0xffffffff >> 1` at 32 bits a positive `0x7fffffff` instead of staying
    // all-ones. The unsigned shift is right on the masked value by definition.
...
/// Sink a definition into its single use and drop the definition.
///
/// `sp` is the architecture's stack pointer, and its definitions are exempt for
/// the same reason [`dce_round`] exempts them: the *statement* is the effect,
/// not just the value. `sub rsp, 0x20` has exactly one use — the flags the
/// `sub` also writes — so it was inlined into a comparison nobody read and the
/// frame allocation disappeared, leaving the stack pointer eight bytes below
/// entry for the rest of the function. Keeping such a definition out of DCE was
/// not enough; this pass removes statements too, and it got there first.
fn expr_prop_round(
    blocks: &mut [SsaBlock],
    ...
    let mut changed = false;
    let is_sp = |name: &str| match sp {
        Some(sp) => name.split('.').next().unwrap_or(name) == sp,
        None => false,
    };
    ...
    for b in blocks.iter_mut() {
        ...
        'stmts: while i < b.stmts.len() {
            ...
            // A call-clobber marker is not a value — it is an assertion about
            // one named register after one call. Inlining it into a use throws
            // the register's name away and leaves an `Unknown` in the middle of
            // an expression, which no consumer can interpret and which reads as
            // "the lift failed here" rather than "the ABI says this is gone".
            let is_clobber =
                matches!(&def_expr, MicroExpr::Unknown(u) if u == n0xis_arch::CALL_CLOBBER);
            if is_sp(&dst) || is_clobber || use_counts.get(&dst).copied().unwrap_or(0) != 1 {
                i += 1;
                continue;
            }
...
/// Remove definitions nothing reads.
///
/// `sp` is the architecture's stack pointer, and a definition of it is **never**
/// dead however few expressions name it. It has an effect beyond its `dst`, the
/// same way a `Store` or a `Call` does: it moves the boundary between this
/// frame and the next one. A function whose locals are addressed off the frame
/// pointer reads `rsp` nowhere after the prologue, so `sub rsp, 0x18` counted as
/// dead and went — and then a call placed the callee's frame, starting with the
/// return address, on top of a live local of the caller. Nothing in the tool
/// could notice: every pass above the lift agreed, because every pass above the
/// lift had also forgotten the frame existed. It took executing the IR against
/// the processor to see it, on a function that adds up eight numbers in a loop.
///
/// The rendered text is unaffected: `RenderNames::is_frame_bookkeeping` already
/// hides an assignment to the stack pointer built from the stack pointer and
/// constants, which is exactly the shape being kept here.
fn dce_round(
    blocks: &mut [SsaBlock],
    delta: &mut Vec<OptDeltaEntry>,
    sp: Option<&'static str>,
    ...
    // The address an indirect jump computes has no *reader* in the IR — the
    // terminator consumes it — so use-counting calls it dead. It is the only
    // record of where a switch actually goes, and removing it takes the
    // dispatch's destination out of the program.
    let is_jump = |name: &str| {
        name.split('.').next().unwrap_or(name) == n0xis_arch::JUMP_TARGET_VAR
    };
    ...
    for b in blocks.iter_mut() {
        ...
        b.stmts.retain(|s| match &s.stmt {
            MicroStmt::Assign { dst, .. } => {
                let live =
                    is_sp(dst) || is_jump(dst) || use_counts.get(dst).copied().unwrap_or(0) > 0;
                if !live {
                    delta.push(OptDeltaEntry {
                        pass: "dce",
                        at: Some(s.va),
        ...
        b.phis.retain(|phi| {
            let live = !phi.dst.is_empty()
                && (is_sp(&phi.dst)
                    || is_jump(&phi.dst)
                    || use_counts.get(&phi.dst).copied().unwrap_or(0) > 0);
            if !live {
                delta.push(OptDeltaEntry {
                    pass: "dce",
                    at: None,
