...
static KNOWN_NORETURN: &[&str] = &[
    ...
    "_Exit",
    "quick_exit",
    // `_cexit` and `_c_exit` are deliberately absent. They run the same
    // cleanup as `exit` and then **return to the caller** — that is the whole
    // difference between them and `exit`, and MSVC documents it. Listing
    // `_cexit` here made every instruction after a call to it unreachable, so
    // the decompiler dropped live code and said nothing. Found because the
    // fixpoint, once it could see import stubs, proved the `_cexit` stub
    // noreturn and put a function that plainly returns on the list.
    "_amsg_exit",
    ...
    "_CxxThrowException",
    // `std::terminate` as the MSVC CRT exports it: the plain name and the
    // VCRUNTIME wrapper every `noexcept` violation and unhandled throw funnels
    // into. The Itanium spelling (`_ZSt9terminatev`) is below; a PE resolves
    // the same function under these.
    //
    // `terminate` is a generic word, and a program's own function of that name
    // would match — the same accepted risk `abort`, `exit` and `err` already
    // carry. It earns it: on one C++ runtime DLL this name is what stands
    // between the fixpoint and every `throw` helper in the image.
    //
    // Deliberately *not* here: `__CxxFrameHandler3`. It is the exception
    // personality routine and it **returns** — `ExceptionContinueSearch` is its
    // ordinary answer for a frame that does not handle the exception. An
    // independent function table calls it noreturn; that is the table being
    // wrong, and following it would prune live code after every EH call site.
    "terminate",
    "__std_terminate",
    // --- x86/x64 fail-fast ---
    "__fastfail",
...
mod tests {
    ...
    /// The MSVC spelling of the same thing. A PE resolves `std::terminate`
    /// under the plain name and under VCRUNTIME's wrapper, and the Itanium
    /// mangling never appears there — so a list carrying only `_ZSt9terminatev`
    /// left every `throw` helper in a C++ runtime DLL unproven.
    #[test]
    fn matches_the_msvc_termination_names_a_pe_target_calls() {
        assert!(is_known_noreturn("terminate"));
        assert!(is_known_noreturn("__std_terminate"));
        assert!(is_known_noreturn("_CxxThrowException"));
        // The personality routine returns `ExceptionContinueSearch` for a frame
        // that does not handle the exception. Marking it noreturn would prune
        // live code after every EH call site.
        assert!(!is_known_noreturn("__CxxFrameHandler3"));
        // `_cexit` runs `exit`'s cleanup and returns; that is what makes it not
        // `exit`. Calling it noreturn deleted every instruction after the call.
        assert!(!is_known_noreturn("_cexit"));
        assert!(!is_known_noreturn("_c_exit"));
        assert!(is_known_noreturn("exit"), "…while `exit` itself does terminate");
    }
