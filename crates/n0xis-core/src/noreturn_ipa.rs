...
pub struct NoReturnArtifact {
    ...
    pub rounds: usize,
    /// How many functions the image has, when the caller knows — so a partial
    /// answer says so.
    ///
    /// This is a **whole-program** fixpoint, and it was reporting
    /// `considered: 4096` on a 15 470-function image with nothing to say that
    /// the other 11 374 were never looked at. A caller whose callee fell
    /// outside the window silently kept its dead fall-through. An incomplete
    /// result is fine; one that cannot be told from a complete one is not.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub total: Option<usize>,
    /// `true` when `considered` is fewer than the functions available.
    pub truncated: bool,
}
