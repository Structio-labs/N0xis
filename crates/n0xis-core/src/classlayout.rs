...
/// A different question from [`crate::ir::symbol_on_entry`]: this wants the
/// **class** a mangled member-function name belongs to. An import has no class,
/// so the stub fallback could only cost a read and a decode per method.
...
///
/// Two grounds, both of them ground truth rather than inference:
///
...
///
/// There was a third: *the qualified name is `Class::method` and RTTI recovered
/// a vtable for `Class`*. It was there to keep a namespaced free function
/// (`Ui::doSomething`) out where the mangling says nothing, and it did — while
/// letting through the thing it could not see. **A static member function of a
/// polymorphic class has exactly that shape**: `Virt::no_this(int)` mangles as
/// `_ZN4Virt7no_thisEi`, `Virt` has a vtable, and the first argument is an
/// `int`. The rule put a `Virt *` there, and a false `this` is not one wrong
/// line — it is a program-wide type name that whole-program propagation then
/// carries to a fixpoint. Measured against `DW_AT_object_pointer`, which is the
/// compiler stating for each function it emitted which parameter is `this`:
/// the ground was right 3 times and wrong once on a 22-function corpus, and
/// removing it trades three recovered `this` pointers for zero invented ones.
///
/// What would restore those three soundly is *membership in the vtable* rather
/// than a name that matches its class: a virtual method is necessarily an
/// instance method. On a PE the slots are in the image and readable. On an ELF
/// shared object they are **relocations** — `R_X86_64_64` entries in
/// `.rela.dyn` naming the method symbol, with zeroes in the file — so it needs
/// the relocation table read into the vtable region, which is the same work
/// devirtualization needs on ELF and is not this change.
///
/// Public because it is a question worth asking from outside, and because a
/// rule this consequential should be measurable directly rather than only
/// through whatever it happens to feed. Every `this` it recovers is a seed the
/// whole-program type propagation can carry, and every one it invents is a
/// wrong type propagated confidently — so the ratio between those two is a
/// number, and `cxx_this_matches_what_the_compiler_recorded` is where it is
/// taken.
pub fn this_class_of(ctx: &Ctx, va: Va) -> Option<String> {
...
fn extract(ctx: &Ctx, va: Va, max_bytes: usize) -> Option<MethodFacts> {
    ...
    let types = TypeInferPass.run(ctx, TypeInferInput { cfg, blocks: opt.blocks.clone(), float_return: opt.float_return, float_return_pair: opt.float_return_pair }).ok()?;
        ...
        impl SymbolProvider for Syms {
            fn symbol_at(&self, va: Va) -> Option<Symbol> {
                let name = match va.0 {
                    ...
                    // A **const** member: the cv-qualifier is what proves it is
                    // not a static member function, and a static member of a
                    // class with a vtable was exactly the case the old
                    // vtable-keyed rule got wrong.
                    0x3000 => "_ZNK6Widget5paintEv",
