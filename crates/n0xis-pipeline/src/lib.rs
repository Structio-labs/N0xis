...
pub fn cfg_cached(ctx: &Ctx, input: CfgInput) -> Result<(CfgArtifact, bool), CoreError> {
    ...
    // The decoder is part of the identity of the answer, not a rendering
    // choice: the same bytes read as x86-64 and as AArch64 are two different
    // programs. Without it the cache returned whichever architecture was asked
    // for first — an ARM64 image analysed as x64 and then, correctly, as arm64
    // got the x64 garbage both times, with no error and no way to tell.
    let scope = format!(
        "{}|{}{}",
        ctx.source.label(),
        ctx.arch.decoder_id(),
        if fingerprint.is_empty() { String::new() } else { format!("|{fingerprint}") }
    );
...
mod tests {
    ...
    /// The cache must not answer one decoder's question with another's answer.
    ///
    /// The key was the source label, the input and the probed bytes — nothing
    /// about the instruction set. So the same image analysed as x86-64 and then
    /// as AArch64 got whichever was asked for **first**, both times, with no
    /// error: an ARM64 library came back full of `jnp` and `div dword ptr`, and
    /// passing `--arch arm64` changed nothing because the answer was already
    /// cached. Two decoders reading the same bytes are two different programs.
    #[test]
    fn the_cache_key_tells_two_decoders_apart() {
        use n0xis_arch::Arch;
        // `A9BF7BFD` is `stp x29, x30, [sp, #-16]!` to AArch64 and a perfectly
        // decodable — and entirely different — x86 sequence.
        let bytes = vec![0xfd, 0x7b, 0xbf, 0xa9, 0xc0, 0x03, 0x5f, 0xd6];
        let snap = Snapshot::builder().region(Va(0x1000), bytes).label("snapshot:arch").build();
        let x64 = X64::new();
        let arm = n0xis_arch::Arm64::new();
        assert_ne!(
            x64.decoder_id(),
            arm.decoder_id(),
            "two decoders that share an id cannot be told apart by any cache"
        );
        // The 32-bit and 64-bit x86 decoders are the same type and share
        // `name()`; only the id separates them.
        assert_ne!(X64::new().decoder_id(), X64::x86().decoder_id());

        let input = CfgInput { start: Va(0x1000), max_bytes: 8, auto_end: true };
        let a = cfg_cached(&Ctx::new(&snap, &x64), input).unwrap().0;
        let b = cfg_cached(&Ctx::new(&snap, &arm), input).unwrap().0;
        let text = |art: &CfgArtifact| -> Vec<String> {
            art.blocks.iter().flat_map(|bl| bl.insns.iter().map(|i| i.text.clone())).collect()
        };
        assert_ne!(
            text(&a),
            text(&b),
            "the same bytes decoded by two instruction sets must not share a cache entry"
        );
    }
