...
pub trait MemorySource {
    ...
    ///
    /// `.text` is not always where the code is. An IL2CPP build puts the
...
pub trait SymbolProvider {
    ...
    /// If `va` is a **thunk to a function defined in this same image**, that
    /// function's address.
    ///
    /// A shared object calls its own exported functions **through the PLT** —
    /// the linker routes them that way so `LD_PRELOAD` can interpose. So in any
    /// `-fPIC` build, an intra-image call reads as `call <stub>` and the stub is
    /// not a function body: every whole-program pass that keys on the callee's
    /// address sees an edge into nothing and drops it. Measured: on a C++ shared
    /// library the type-propagation pass found **zero** call edges between the
    /// corpus functions, and propagated nothing, for exactly this reason.
    ///
    /// `None` for a real import (the definition is in another module) and for
    /// anything that is not a thunk — so a consumer that ignores this is no
    /// worse off than before, and one that follows it sees the program's own
    /// call graph.
    fn thunk_to(&self, _va: Va) -> Option<Va> {
        None
    }
...
impl SymbolProvider for ChainedSymbols<'_> {
    ...
    /// Same shape: only the image knows its own relocations, so this forwards
    /// rather than answering. **Forwarding is not optional** — a chained or
    /// wrapped provider that keeps the trait default silently reports "not a
    /// thunk" for every address, and the caller cannot tell that apart from an
    /// image with no thunks. That is how this method's first version reached a
    /// green build while doing nothing.
    fn thunk_to(&self, va: Va) -> Option<Va> {
        self.primary.thunk_to(va).or_else(|| self.fallback.thunk_to(va))
    }
