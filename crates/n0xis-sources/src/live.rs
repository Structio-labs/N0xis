...
impl LiveProcess {
    ...
    /// for a detour/trampoline hook (ROADMAP Phase 4b), letting the OS pick the
    /// address.
    ///
    /// **Prefer [`alloc_code_cave_near`](Self::alloc_code_cave_near).** The
    /// unhinted form was documented as "*usually* within a `jmp rel32`'s reach
    /// of a nearby module, but never guaranteed". Measured on Windows 11 it is
    /// never in reach: a 64-bit image loads around `0x7ff6_xxxx_xxxx` and an
    /// unhinted `VirtualAllocEx` returns around `0x1ce8_xxxx_xxxx` — some 29 GB
    /// away, against the ±2 GB a near jump can encode. `patch detour` therefore
    /// refused on every ordinary 64-bit target: correctly, since the
    /// alternative is writing a jump to the wrong place, but the feature could
    /// not be used at all.
    ...
    /// Allocate a code cave **within `jmp rel32` reach of `near`**.
    ///
    /// A near jump encodes a signed 32-bit displacement, so the cave has to
    /// live within ±2 GB of the hook site. Windows will not do that on its own:
    /// it needs an `lpAddress` hint, and the hint must name a free region
    /// aligned to the allocation granularity. So the free list is walked
    /// outward from `near` — both directions, closest first — and the first
    /// region that takes the allocation wins.
    ///
    /// Falls back to the unhinted allocation when no free region is in range,
    /// which leaves the caller exactly where it was before: holding a cave it
    /// must still range-check.
    pub fn alloc_code_cave_near(&self, near: Va, size: usize) -> Result<Va, SourceError> {
        // The reach of `jmp rel32`, with a margin for the instruction's own
        // length and the trampoline's tail.
        const REACH: u64 = 0x7fff_0000;
        const GRANULARITY: u64 = 0x1_0000; // 64 KiB, Windows' allocation unit

        let lo = near.0.saturating_sub(REACH);
        let hi = near.0.saturating_add(REACH);

        // Collect the free regions in range, then try them nearest-first: a
        // closer cave keeps more of the ±2 GB budget for the jump back.
        let mut free: Vec<u64> = Vec::new();
        let mut probe = lo & !(GRANULARITY - 1);
        while probe < hi {
            let Some(mbi) = self.query(probe) else { break };
            let base = mbi.BaseAddress as u64;
            let region = mbi.RegionSize as u64;
            if region == 0 {
                break;
            }
            if mbi.State == MEM_FREE && region >= size as u64 {
                // Every granularity-aligned slot in this free region is a
                // candidate; the ends are what matter, so take both.
                let first = (base + GRANULARITY - 1) & !(GRANULARITY - 1);
                let last = (base + region - size as u64) & !(GRANULARITY - 1);
                for cand in [first, last] {
                    if cand >= lo && cand + size as u64 <= hi && cand >= base && cand + (size as u64) <= base + region {
                        free.push(cand);
                    }
                }
            }
            probe = base.saturating_add(region);
        }
        free.sort_by_key(|a| a.abs_diff(near.0));
        free.dedup();
        ...
        for cand in free {
            ...
            if !addr.is_null() {
                return Ok(Va(addr as u64));
            }
        }
        self.alloc_code_cave(size)
    }
...
/// "where is the code", which on an IL2CPP image is a different question:
...
impl LiveTarget for LiveProcess {
    ...
    fn alloc_code_cave_near(&self, near: Va, size: usize) -> Result<Va, SourceError> {
        LiveProcess::alloc_code_cave_near(self, near, size)
    }
