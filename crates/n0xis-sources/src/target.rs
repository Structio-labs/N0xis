...
pub trait LiveTarget: MemorySource + ModuleProvider + SymbolProvider {
    ...
    ///
    /// A near jump encodes a signed 32-bit displacement, so a trampoline is
    /// only writable when its cave is within ±2 GB of the hook site. Letting
    /// the OS choose is not good enough: measured on Windows 11, a 64-bit image
    /// at `0x7ff6_xxxx_xxxx` and an unhinted allocation at `0x1ce8_xxxx_xxxx`
    /// are 29 GB apart, so `patch detour` refused on every ordinary target.
    ///
    /// The default ignores the hint, which is the old behaviour: an adapter
    /// that cannot place an allocation is no worse off than before, and the
    /// caller still range-checks.
    fn alloc_code_cave_near(&self, _near: Va, size: usize) -> Result<Va, SourceError> {
