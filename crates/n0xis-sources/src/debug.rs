...
use windows_sys::Win32::System::Diagnostics::Debug::{
    CONTEXT, CONTEXT_FULL_AMD64, ContinueDebugEvent, DEBUG_EVENT, DebugActiveProcess,
...
fn capture_hit(
    ...
    if unsafe { GetThreadContext(h_thread, &mut ctx) } == 0 {
        return Err(SourceError::Os(format!("GetThreadContext({tid}) failed")));
    }
...
fn rewind_rip(tid: u32, bp_va: Va) -> Result<(), SourceError> {
    ...
    ctx.Rip = bp_va.0;
    if unsafe { SetThreadContext(h_thread, &ctx) } == 0 {
