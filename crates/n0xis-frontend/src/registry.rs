...
//! spawned through `capability run plugin.<name>`). Two mechanisms, and the
//! built-in one privileged.
//!
...
/// Attach the user's per-address comments (`.n0x/annotations.json`) to the rows
/// that carry one.
///
/// It lived in the CLI's own copy of this command, so a comment showed in the
/// terminal listing and in nothing else — the MCP server and `serve` reach the
/// same command through the registry and saw none of them. One command with two
/// implementations is one command with two answers.
///
/// Silent (never an error) when there is no project or nothing is commented, so
/// a `--bytes` source or a directory outside a project simply carries none.
fn attach_comments(val: &mut Value) {
    let Ok(records) = n0xis_project::annotate::list() else { return };
    let comments: std::collections::BTreeMap<u64, String> = records
        .into_iter()
        .filter_map(|r| r.comment.filter(|c| !c.trim().is_empty()).map(|c| (r.va.0, c)))
        .collect();
    if comments.is_empty() {
        return;
    }
    let Some(insns) = val.get_mut("insns").and_then(|v| v.as_array_mut()) else { return };
    for insn in insns {
...
/// [`spec_of`], with the base inline `bytes` are mapped at defaulted to the
/// address being asked about.
///
/// Inline bytes have no image and so no base of their own, which leaves exactly
/// one sensible answer: they start where the caller says to look.
/// [`with_cfg_ctx`] did that and the hand-written `decode` did not, so
/// `disasm --bytes "48 89 c8 c3" --addr 0x1000` decoded two instructions from
/// the CLI and answered `decode-failed` through MCP — one command, two
/// implementations, two answers.
fn spec_at(args: &Value, start: Va) -> SourceSpec<'_> {
    let mut spec = spec_of(args);
    if spec.bytes_base.is_none() {
        spec.bytes_base = Some(start);
    }
    spec
}

/// Refuse an address the source cannot hold, with the reason the *kind* of
/// source makes true: a static image has sections, a live process has committed
/// regions. Reading past either is not a decode failure, and reporting it as
/// one loses the only thing the caller can act on.
fn refuse_unreadable(src: &Src, addr: Va) -> Option<Response<Value>> {
    if src.as_mem().contains(addr) {
        return None;
    }
    Some(match src {
        Src::Live(_) => Response::error("addr-not-committed", format!("{addr} is not a committed, readable region in this process"))
            .with_hint("use a runtime VA (respecting ASLR); `n0xis process ps` finds the pid"),
        _ => {
            let hint = match src.module_base() {
                Some(base) => format!("pass a VA at the preferred image base {base} (ASLR'd modules need --pid)"),
                None => "the address lies outside every mapped region of this source".to_string(),
            };
            Response::error("addr-out-of-image", format!("{addr} is outside any section of the image")).with_hint(hint)
        }
    })
}
...
///
/// A dispatch through a field's declared type is sound as a *type* and unsound
/// as a *vtable*: what is stored there may be a derived class, whose slot holds
/// a different method. Knowing which classes are subclassed at all is what lets
/// [`n0xis_core::devirtualize`] refuse exactly those and keep the rest.
///
/// Built from the same RTTI scan that recovers the vtables — MSVC's hierarchy
/// descriptors, or the Itanium `type_info` chain. Memoised per image the way
        ...
        _ => match src {
            Src::Static(img) => match img.as_ref() {
                n0xis_sources::StaticImage::Elf(elf) => {
                    n0xis_core::scan_itanium_rtti(src.as_mem(), &elf.data_symbols(), src.text_range())
                }
                _ => Vec::new(),
            },
            _ => Vec::new(),
    ...
    let map: std::collections::HashMap<u64, String> = if !persisted.is_empty() {
        ...
        match (src.module_base(), src.section_range(".rdata")) {
            (Some(base), Some(_)) => n0xis_core::scan_msvc_rtti(src.as_mem(), base, &src.rtti_data_ranges(), src.text_range(), src.pointer_size())
                ...
                .collect(),
            // No `.rdata` means no MSVC RTTI, which used to mean no vtable map
            // at all — so devirtualization never ran on an ELF. It does not
            // need MSVC RTTI: the Itanium ABI names every vtable outright.
            _ => itanium_vtable_map(src),
        }
    };
...
/// How many instructions a linear disassembly returns when the caller names no
/// count.
///
/// One constant because there are two front doors onto the same operation —
/// `n0x disasm` and the `decode` capability (which is what an MCP client
/// reaches) — and they had disagreed: 20 against 16, so the same request
/// answered differently depending on which door it came through. A default is
/// part of the contract, and a contract with two implementations needs one
/// source for it.
...
/// already used — but every command that had not been through an `analyze`
/// reached this map instead, and this map only knew how to read MSVC RTTI out
/// of `.rdata`. So devirtualization was silently unavailable on an ELF unless
/// the project happened to carry a persisted scan, and a fresh checkout of a
/// 22 415-function target resolved nothing.
fn itanium_vtable_map(src: &Src) -> std::collections::HashMap<u64, String> {
    let Src::Static(img) = src else { return std::collections::HashMap::new() };
    let n0xis_sources::StaticImage::Elf(elf) = img.as_ref() else {
...
/// Every function extent and protected range the image states, read from
/// whichever unwind table it carries. `None` means it carries neither, which is
/// the one case a caller can report as an error rather than an empty answer.
///
/// **Which table is asked first is not a matter of taste.** A PE states its
/// function extents in `.pdata`, and that is authoritative whether or not the
/// same image also carries `.eh_frame` — which every GCC-family toolchain puts
/// there. Asking `.eh_frame` first on a PE therefore answered for the images
/// this project builds and tests against: 0 functions of the 61 in a mingw
/// build's own table, 273 of 4 270 in one mingw-built runtime DLL, 87 of 1 750
/// in another. `DiscoverPass` takes its declared extents from here, so the
/// same mistake also let a prologue match two bytes inside a real function be
/// reported as a function of its own.
///
/// When an image has both, `.pdata` settles the function set and `.eh_frame`
/// still contributes the protected ranges it describes — neither table is a
/// superset of the other.
fn scan_unwind(src: &Src) -> Option<Vec<n0xis_core::EhFunction>> {
    let eh_frame = src.section_range(".eh_frame");
    let pdata = match (src.section_range(".pdata"), pe_image_base(src)) {
        (Some(range), Some(base)) => Some((range, base)),
        _ => None,
    };
    match (pdata, eh_frame) {
        (Some((range, base)), eh) => {
            let mut fns = n0xis_core::scan_pdata(src.as_mem(), base, range);
            if let Some(eh) = eh {
                merge_regions(&mut fns, n0xis_core::scan_eh_frame(src.as_mem(), eh));
            }
            Some(fns)
        }
        (None, Some(eh)) => Some(n0xis_core::scan_eh_frame(src.as_mem(), eh)),
        (None, None) => None,
    }
}

/// Fold the protected ranges `extra` describes into whichever function in
/// `into` actually covers them, dropping a range no function covers and one
/// already recorded. `into` keeps its own extents — only regions are added.
fn merge_regions(into: &mut [n0xis_core::EhFunction], extra: Vec<n0xis_core::EhFunction>) {
    if extra.is_empty() {
        return;
    }
    into.sort_unstable_by_key(|f| f.start.get());
    for f in extra {
        for r in f.regions {
            let i = match into.binary_search_by_key(&r.try_start.get(), |f| f.start.get()) {
                Ok(i) => i,
                Err(0) => continue,
                Err(i) => i - 1,
            };
            let host = &mut into[i];
            if r.try_end.get() > host.end.get() || host.regions.contains(&r) {
                continue;
            }
            host.regions.push(r);
        }
...
/// `DiscoverPass` seeds it and leaves the extent to be inferred, which is
/// exactly what an export is: the file's own word that a function begins here,
/// and nothing about how long it is.
///
/// **Public because it must have one definition.** The CLI kept its own copy of
/// the `.eh_frame`/`.pdata` half, so when exports were added here the two front
/// doors answered differently about the same image — 2 394 functions through
/// one and 2 396 through the other. A fact the file states is not a per-frontend
/// opinion.
...
/// A remark when the requested address is inside the image but in **no section
/// the image marks executable**.
///
/// Asked about `0x100b9000` — the middle of a PE's export directory — every
/// code command answered `ok: true` and none of them mentioned that the bytes
/// are not code: `decomp pseudo` returned `void sub_100b9000()` with
/// `quality: 0.67`, `function summary` reported a complete clobber set, and
/// the section table saying `READONLY, DATA` was in the same file the whole
/// time. An address arrives from a pointer, a vtable slot or an arithmetic
/// slip as often as from a function list, and a plausible function is the
/// worst possible answer to "what is at this address".
///
/// A **note**, not a refusal, on purpose: a packer puts real code in a data
/// section, and unpacking is a thing this tool is meant to do (ROADMAP Phase
/// 15). Disassembling data is a legitimate request; not being told is not.
///
/// Silent when the source declares no executable range at all — `--bytes` and
/// a snapshot know of none, and absence of knowledge is not evidence.
fn outside_code_note(src: &Src, start: Va) -> Option<String> {
    let ranges = src.as_mem().code_ranges();
    if ranges.is_empty() || ranges.iter().any(|(va, len)| start.get() >= va.get() && start.get() < va.get() + len) {
        return None;
    }
    Some(format!(
        "{start} is not in any section this image marks executable; the bytes are decoded as code because they were asked for, not because they are"
    ))
}

/// Add a remark without discarding one the capability already made. `with_note`
/// replaces, and a capability that explains its own result (`decomp pseudo`
/// does) would silently lose that explanation.
fn add_note(mut resp: Response<Value>, note: Option<String>) -> Response<Value> {
    let Some(note) = note else { return resp };
    if let Response::Ok(ok) = &mut resp {
        ok.meta.note = Some(match ok.meta.note.take() {
...
/// Spell every register a response names the way the **target** spells it, at
/// the one boundary every front door shares.
///
/// The canonical register name is a 64-bit spelling on x86. That is right as an
/// *identity* — `al`, `ax`, `eax` and `rax` are one register, and every join
/// depends on them sharing one token — and false as a *claim* on a 32-bit
/// image, where `rax` and `rsp` do not exist. The mismatch shipped: one
/// `ir build` object carried `"text":"sub esp,0Ch"` beside `"reads":["rsp"]`
/// (the same register, two names, adjacent fields), `function summary`
/// answered `clobbers:["rax"]`, `ir value-set` reported sets for
/// `rax`/`rcx`/`rdx`/`rsp`, and `decomp pseudo` rendered `rax.1 = …` — all for
/// an i386 binary whose own disassembly said `esp`.
///
/// It runs **here**, after the analysis and before the JSON, for two reasons.
/// The analyses must keep the canonical name: the backward slice matches a
/// user's `--reg` against exactly these strings after
/// [`Arch::normalize_reg`](n0xis_arch::Arch::normalize_reg), so renaming at
/// construction would leave it hunting a token no record carries. And doing it
/// once for every capability makes the affected set a *measured* property
/// rather than a claim — a fifth command that reports a register is covered
/// without being found first, and cannot regress quietly.
///
/// Only a whole string that **is** a register name is rewritten — `"rsp"`, or
/// a versioned SSA name like `"rax.1"` — never a substring, so a disassembly
/// line, a symbol or a comment containing those letters is untouched. Object
/// *keys* are rewritten too: `ir value-set` reports its sets in a map keyed by
/// SSA name. A target whose canonical names already are its own has an empty
/// [`Arch::display_reg_map`](n0xis_arch::Arch::display_reg_map) and pays a
/// single `is_empty`.
fn spelled(arch: &dyn n0xis_arch::Arch, mut resp: Response<Value>) -> Response<Value> {
    let map = arch.display_reg_map();
    if map.is_empty() {
        return resp;
    }
    if let Response::Ok(ok) = &mut resp {
        walk(map, &mut ok.data);
    }
    return resp;

    /// `rsp` → `esp`, `rax.1` → `eax.1`; `None` when the string is not a
    /// register name this target spells differently.
    fn spell(map: &[(&'static str, &'static str)], s: &str) -> Option<String> {
        let (stem, version) = match s.split_once('.') {
            Some((stem, v)) if !v.is_empty() && v.chars().all(|c| c.is_ascii_digit()) => (stem, Some(v)),
            Some(_) => return None,
            None => (s, None),
        };
        let target = map.iter().find(|(c, _)| *c == stem)?.1;
        Some(match version {
    ...
    fn walk(map: &[(&'static str, &'static str)], v: &mut Value) {
        match v {
            Value::String(s) => {
                if let Some(t) = spell(map, s) {
                    *s = t;
                }
            }
            Value::Array(items) => items.iter_mut().for_each(|i| walk(map, i)),
            Value::Object(o) => {
                let renamed: Vec<(String, String)> =
                    o.keys().filter_map(|k| spell(map, k).map(|t| (k.clone(), t))).collect();
                for (from, to) in renamed {
                    if let Some(val) = o.remove(&from) {
                        o.insert(to, val);
                    }
                }
                o.values_mut().for_each(|i| walk(map, i));
            }
            _ => {}
        }
    }
...
fn with_cfg_ctx(args: &Value, work: impl FnOnce(&n0xis_core::Ctx, n0xis_core::CfgInput, &str) -> Response<Value>) -> Response<Value> {
    ...
    // (the module base comes from it), so inline bytes fall back to base 0 —
    // they have no module and are rejected below anyway.
    let spec = spec_at(args, if addr_rva { Va(0) } else { addr });
    ...
    let arch = match crate::pick_arch_for(
        args.get("arch").and_then(|v| v.as_str()),
        resolved.src.declared_machine().as_deref(),
        !resolved.src.is_64(),
    ) {
        Ok(a) => a,
    ...
    // long-standing "no symbols on --pid" blind spot, closed here for IL2CPP
    ...
    let subclassed = subclassed_classes(&resolved.src);
        ...
        Src::Static(pe) => {
            ...
            work(
                &n0xis_core::Ctx::new(pe.as_ref(), arch.as_ref())
                    ...
                    .with_subclassed(&subclassed)
                    .with_eh(eh_regions)
                    .with_stated_functions(&stated)
                    .with_type_flow(&flow)
                    .with_layout(&layout),
                input,
                &label,
    ...
    let resp = add_note(resp, outside_code_note(&resolved.src, start));
    spelled(&*arch, attach.annotate(resp))
}
...
fn with_src_ctx(
    ...
    let subclassed = subclassed_classes(&src);
    ...
    let resp = match &src {
        Src::Static(pe) => {
            ...
            // stretched an 82-byte function to the whole decode window.
            let stated = stated_functions(&src);
            work(
                &n0xis_core::Ctx::new(pe.as_ref(), arch.as_ref())
                    .with_symbols(&full)
                    .with_modules(pe.as_ref())
    ...
    fn register(&self, reg: &mut Registry) {
        reg.add(Capability::new(
            ...
            Box::new(|args| {
                ...
                let resolved = match resolve(spec_at(args, addr)) {
                    Ok(r) => r,
                    Err((c, m)) => return Response::error(&c, m),
                };
                if let Some(refusal) = refuse_unreadable(&resolved.src, addr) {
                    return refusal;
                }
                let arch = match crate::pick_arch_for(
        args.get("arch").and_then(|v| v.as_str()),
        ...
        !resolved.src.is_64(),
                ...
                let input = n0xis_core::DecodeInput::count(addr, usize_arg(args, "count", DEFAULT_DECODE_COUNT));
                let note = outside_code_note(&resolved.src, addr);
                match n0xis_core::Pass::run(&n0xis_core::DecodePass, &ctx, input) {
                    Ok(out) => match serde_json::to_value(out) {
                        Ok(mut v) => {
                            attach_comments(&mut v);
                            add_note(
                                Response::success(n0xis_contracts::schema::v1::DECODE, v).with_source(resolved.label),
                                note,
                            )
                        }
                        Err(e) => Response::error("serialize", e.to_string()),
        ...
        reg.add(Capability::new(
            ...
            Box::new(|args| {
                ...
                // The same reader `stated_functions` uses. It was two readers,
                // and on a PE carrying both tables they answered differently
                // about the same image.
                let Some(all) = scan_unwind(&resolved.src) else {
        ...
        reg.add(Capability::new(
            ...
            Box::new(|args| {
                ...
                with_src_ctx(args, Va(0), move |ctx, src, _region_len, label| {
                    ...
                    // Only when the caller named the address. Without one this
                    // summarizes the *discovered* set, every member of which is
                    // in a code range by construction.
                    let note = explicit.and_then(|a| outside_code_note(src, a));
        ...
        // --- data-side scanning, sharing `with_scan_ctx` ---
                    ...
                    args.get("pid").and_then(|v| v.as_u64()).map(|v| v as u32)
                {
                    match crate::source::attach_live(pid) {
                        Ok(live) => {
                            ...
                            let mods = n0xis_sources::ModuleProvider::modules(live.as_ref()).to_vec();
                            let mut art = None;
                            // Why each module was rejected, deduplicated. Throwing
                            // these away left one generic "no metadata found" on a
                            // Linux NativeAOT target whose image demonstrably
                            // carries the `RTR\0` header — the true answer was
                            // "this reader parses PE modules", which is a different
                            // thing from "there is nothing here".
                            ...
                            for m in &mods {
                                ...
                                considered += 1;
                                match n0xis_core::parse_aot(live.as_ref(), m.base) {
                                    Ok(a) => {
                                        art = Some(a);
                                        break;
                                    }
                            ...
                            match art {
                                ...
                                None => Err(Response::error(
                                    "no-aot",
                                    format!(
                                        "no NativeAOT stack-trace metadata read from any of {considered} module(s); \
                                         each was rejected because: {}. Name the module with `module` if the target \
                                         is not the first with a ReadyToRunHeader",
                                        if reasons.is_empty() { "no module matched the filter".to_string() } else { reasons.join("; ") }
                                    ),
                                )),
                            }
                        }
        ...
        reg.add(Capability::new(
            ...
            Origin::Builtin,
            Box::new(|args| {
                // Through `with_src_ctx`, like every other capability — it is
                // what attaches the symbol chain (the image's exports, a managed
                // index, FLIRT matches, the user's own renames). This built its
                // own `Ctx` instead, so the function list came back as a wall of
                // `sub_XXXX`: measured on a 32-bit system DLL, **0 of 4 143
                // functions carried a name here against 1 152 through the CLI**,
                // with `ok: true` and nothing to say a name was even possible.
        ...
        !resolved.src.is_64(),
                ...
                // A *whole-program* fixpoint defaults to the whole program.
                // Capping it silently made a partial answer indistinguishable
                // from a complete one; `truncated` now says, and the default
                // no longer contradicts the command's own description.
                let limit = usize_arg(args, "limit", 0);
                let max_bytes = usize_arg(args, "max_bytes", 0x4000);
                let mem = resolved.src.as_mem();

                // Function entry set. `DiscoverPass` first — it already folds in
                // the exception table (`Ctx::functions`) and adds what the table
                // does not describe, which is exactly the nodes this pass needs.
                //
                // It used to take `.pdata` alone, on the reasoning that the
                // alternative was a prologue scan that "misses most of them".
                // That reasoning expired: on one C++ DLL the table has 1 398
                // functions and discovery finds 2 106, and the 708 in between
                // include the **import stubs** — a linker routes a call to an
                // import through a six-byte `jmp [rip+slot]`, so the stub is the
                // node between a thrower and the `_CxxThrowException` it calls.
                // Without those nodes the fixpoint proved 2 functions of 1 398;
                // with them, 28, against 12 an independent function table finds
                // (and every one of the extra 20 is a throw helper or a
                // never-returning catch funclet the table under-reports).
                //
                // The `.pdata` branch stays as the fallback for a source whose
                // executable range cannot be resolved.
                ...
                let funcs: Vec<Va> = 'entries: {
                    ...
                    let Some((start, size)) =
                        ...
                        // No resolvable code range: the exception table is the
                        // only entry set left.
                        ...
                        {
                            available = all.len();
                            let take = if limit == 0 { available } else { limit };
                            break 'entries all.into_iter().take(take).map(|f| f.va).collect();
                        }
                        break 'entries Vec::new();
                    };
                    let dstated = stated_functions(&resolved.src);
                    let dctx = n0xis_core::Ctx::new(mem, arch.as_ref()).with_stated_functions(&dstated);
                    let di = n0xis_core::DiscoverInput { start, size, limit, offset: 0 };
                    // Ask for everything, then cap — otherwise `available`
                    // would be the cap and the truncation invisible.
                    let all = n0xis_core::DiscoverInput { limit: 0, ..di };
                    match n0xis_core::Pass::run(&n0xis_core::DiscoverPass, &dctx, all) {
                        Ok(d) => {
                            available = d.functions.len();
                            let take = if limit == 0 { d.functions.len() } else { limit };
                            d.functions.iter().take(take).map(|f| f.va).collect()
                        }
                        Err(_) => Vec::new(),
                    }
                };
                ...
                let run = |ctx: &n0xis_core::Ctx| -> Response<Value> {
                    let mut art = n0xis_core::propagate_noreturn(ctx, &funcs, max_bytes);
                    art.total = (available > 0).then_some(available);
                    art.truncated = available > art.considered;
...
mod tests {
    ...
    /// A PE built with a GCC-family toolchain carries `.eh_frame` **and**
    /// `.pdata`. Which one is asked first is not a matter of taste: `.pdata` is
    /// where a PE states its function extents, and taking `.eh_frame` because
    /// it happened to be present answered for exactly the images this project
    /// builds and tests against — 0 functions of the 61 in a mingw build's own
    /// table, 273 of 4 270 in one mingw-built runtime DLL. `DiscoverPass` reads
    /// its declared extents from here, so that also cost the default
    /// `function discover` its 61-of-61 recall and let a prologue match two
    /// bytes inside a real function pass as a function of its own.
    #[test]
    fn a_pe_carrying_both_unwind_tables_is_read_as_a_pe() {
        let path = std::env::temp_dir().join(format!("n0xis_both_tables_{}.exe", std::process::id()));
        std::fs::write(&path, pe_with_both_tables()).unwrap();
        let img = n0xis_sources::StaticImage::load(&path).expect("the synthetic PE loads");
        let src = Src::Static(std::sync::Arc::new(img));

        let fns = scan_unwind(&src).expect("an image with either table has an answer");
        assert_eq!(
            fns.iter().map(|f| (f.start.get(), f.end.get())).collect::<Vec<_>>(),
            vec![(0x140001000, 0x140001010), (0x140001020, 0x140001040)],
            "the `.pdata` extents, not whatever the `.eh_frame` section holds"
        );
        let _ = std::fs::remove_file(&path);
    }

    /// PE32+ with three sections — `.text`, a two-entry `.pdata`, and an
    /// `.eh_frame` holding bytes that are not a CIE, so a reader that picks it
    /// reports nothing and the difference is unmistakable.
    ///
    /// `.eh_frame` is nine characters and a PE section name is eight, so it is
    /// spelled the way a real mingw image spells it: `/4`, an offset into the
    /// COFF string table. Writing the truncated `.eh_fram` instead gave an
    /// image with no such section at all, and the test then passed against the
    /// unfixed code — which is what calibration is for.
    fn pe_with_both_tables() -> Vec<u8> {
        const E_LFANEW: usize = 0x40;
        const OPT: usize = E_LFANEW + 24;
        const OPT_SIZE: u16 = 240;
        let sec_table = OPT + OPT_SIZE as usize;
        let headers = 0x400usize; // SizeOfHeaders / first section's raw offset
        let mut b = vec![0u8; headers + 3 * 0x200];
        b[0..2].copy_from_slice(b"MZ");
        b[0x3c..0x40].copy_from_slice(&(E_LFANEW as u32).to_le_bytes());
        b[E_LFANEW..E_LFANEW + 4].copy_from_slice(b"PE\0\0");
        b[E_LFANEW + 4..E_LFANEW + 6].copy_from_slice(&0x8664u16.to_le_bytes()); // machine
        b[E_LFANEW + 6..E_LFANEW + 8].copy_from_slice(&3u16.to_le_bytes()); // sections
        b[E_LFANEW + 20..E_LFANEW + 22].copy_from_slice(&OPT_SIZE.to_le_bytes());
        b[E_LFANEW + 22..E_LFANEW + 24].copy_from_slice(&0x22u16.to_le_bytes()); // executable, large-address-aware
        b[OPT..OPT + 2].copy_from_slice(&0x20bu16.to_le_bytes()); // PE32+
        b[OPT + 24..OPT + 32].copy_from_slice(&0x140000000u64.to_le_bytes()); // ImageBase
        ...
        b[OPT + 60..OPT + 64].copy_from_slice(&(headers as u32).to_le_bytes()); // SizeOfHeaders
        b[OPT + 108..OPT + 112].copy_from_slice(&16u32.to_le_bytes()); // NumberOfRvaAndSizes
        // DataDirectory[3] = IMAGE_DIRECTORY_ENTRY_EXCEPTION -> the `.pdata` section
        b[OPT + 112 + 24..OPT + 112 + 28].copy_from_slice(&0x2000u32.to_le_bytes());
        b[OPT + 112 + 28..OPT + 112 + 32].copy_from_slice(&24u32.to_le_bytes());

        // The COFF string table lives past the section data; `/4` names its
        // first entry, which is how a long section name is spelled in a PE.
        let strtab = headers + 3 * 0x200;
        b[E_LFANEW + 12..E_LFANEW + 16].copy_from_slice(&(strtab as u32).to_le_bytes()); // PointerToSymbolTable
        b[E_LFANEW + 16..E_LFANEW + 20].copy_from_slice(&0u32.to_le_bytes()); // NumberOfSymbols
        let long_name = b".eh_frame\0";
        b.extend_from_slice(&((4 + long_name.len()) as u32).to_le_bytes());
        b.extend_from_slice(long_name);

        let sections: [(&[u8], u32, u32, u32); 3] = [
            (b".text\0\0\0", 0x1000, headers as u32, 0x200),
            (b".pdata\0\0", 0x2000, headers as u32 + 0x200, 0x200),
            (b"/4\0\0\0\0\0\0", 0x3000, headers as u32 + 0x400, 0x200),
        ];
        for (i, (name, rva, raw, size)) in sections.iter().enumerate() {
            let h = sec_table + i * 40;
            let n = name.len().min(8);
            b[h..h + n].copy_from_slice(&name[..n]);
            b[h + 8..h + 12].copy_from_slice(&size.to_le_bytes()); // VirtualSize
            b[h + 12..h + 16].copy_from_slice(&rva.to_le_bytes());
            b[h + 16..h + 20].copy_from_slice(&size.to_le_bytes()); // SizeOfRawData
            b[h + 20..h + 24].copy_from_slice(&raw.to_le_bytes());
            b[h + 36..h + 40].copy_from_slice(&0x6000_0020u32.to_le_bytes()); // code, read, execute
        }
        // Two RUNTIME_FUNCTIONs in `.pdata`'s raw bytes.
        let pdata = headers + 0x200;
        for (i, (begin, end)) in [(0x1000u32, 0x1010u32), (0x1020, 0x1040)].iter().enumerate() {
            let at = pdata + i * 12;
            b[at..at + 4].copy_from_slice(&begin.to_le_bytes());
            b[at + 4..at + 8].copy_from_slice(&end.to_le_bytes());
            b[at + 8..at + 12].copy_from_slice(&0u32.to_le_bytes()); // no unwind info
        }
        b
    }

    /// `--limit 0` is this command's default and is documented as "every
    /// function". In the `.pdata` branch it was passed straight to `take`,
    /// which asked for none — so the whole-program analysis refused to run at
    /// all on every x64 PE, which is every image it is built for. It failed
    /// with `no-functions` on a PE whose table holds 3 988 of them.
    #[test]
    fn the_whole_program_noreturn_pass_runs_with_its_default_limit() {
        let path = std::env::temp_dir().join(format!("n0xis_noreturn_limit_{}.exe", std::process::id()));
        std::fs::write(&path, pe_with_both_tables()).unwrap();
        let mut reg = Registry::new();
        reg.add_plugin(&AnalysisPasses);
        let v = serde_json::to_value(reg.dispatch("function.noreturn", &json!({ "file": path.to_str().unwrap() }))).unwrap();
        let _ = std::fs::remove_file(&path);
        assert_eq!(v["ok"], true, "the default limit must not empty the entry set: {v}");
        // Both declared functions, and nothing invented out of the fixture's
        // zero-filled `.text`. The entry set is the full discovery now, not the
        // exception table alone — a whole-program pass needs the nodes the
        // table does not describe (an import stub is one) — so this asserts the
        // table's functions are *included*, not that they are all there is.
        assert_eq!(v["data"]["considered"], 2, "the two declared functions, and no invented ones: {v}");
    }
