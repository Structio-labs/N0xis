...
use n0xis_sources::{MemorySource, ModuleProvider, RemoteAgent, Snapshot, StaticPe, split_command_line};
...
impl Src {
    ...
    /// implement `ModuleProvider`). Centralized so callers never match on
    /// `Src::Live` themselves — that arm only exists on Windows.
...
pub fn resolve(spec: SourceSpec<'_>) -> Result<ResolvedSource, FrontendError> {
    ...
    if let Some(pid) = pid {
        #[cfg(not(windows))]
        {
            ...
            return Err(err(
                "live-unsupported",
                "pid (live-process analysis) requires a Windows build (needs LiveProcess/Win32 APIs)",
            ));
        }
        #[cfg(windows)]
        {
            let live = LiveProcess::attach(pid).map_err(|e| err("attach-failed", e.to_string()))?;
            let label = live.label();
            return Ok(ResolvedSource { src: Src::Live(Box::new(live)), label, region_len: None });
        }
    }
...
#[cfg(windows)]
pub fn live_scan_regions(live: &n0xis_sources::LiveProcess, start: Option<&str>, size: Option<usize>) -> Result<Vec<(Va, usize)>, String> {
    if let Some(s) = start {
