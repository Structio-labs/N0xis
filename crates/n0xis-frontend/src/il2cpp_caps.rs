...
//!
//! - **An IL2CPP WebGL index can never bind to a native target.** The addresses
...
/// two copies of a layout rule drift the moment the runtime changes it.
...
impl IndexAttach {
    pub fn symbols(&self) -> Option<&dyn n0xis_sources::SymbolProvider> {
        match self {
    ...
    pub fn annotate(&self, resp: Response<Value>) -> Response<Value> {
        match self {
...
/// reports instead.
pub fn attach_for(args: &Value, src: &crate::source::Src) -> IndexAttach {
    ...
    fn register(&self, reg: &mut Registry) {
        reg.add(Capability::new(
            ...
            "Import an external IL2CPP dump (Il2CppDumper script.json) as a named symbol index under .n0x/. \
             ...
             IL2CPP WebGL dumps import as searchable name tables and are never bound to a native image.",
        ...
        reg.add(Capability::new(
            "il2cpp.icalls",
            "Recover the engine's internal calls from the code that resolves them: the registration name, and the .data slot \
            ...
            Box::new(|args| {
                ...
                // Measured across three IL2CPP builds: one had 1764 icall names
        ...
        reg.add(Capability::new(
            ...
            Box::new(|args| {
                ...
                // not have to know the IL2CPP directory layout to read the blob
                // sitting next to it.
