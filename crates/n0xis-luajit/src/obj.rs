...
//! data`@0x14`. That engine family is known to patch LuaJIT internals, so we do
//! not assume the stock layout; every magic offset lives in [`LuaLayout`] and
//! is meant to be *calibrated* (see [`LuaLayout::BITSQUID_LUAJIT`], and the
...
impl LuaLayout {
    ...
    pub const BITSQUID_LUAJIT: LuaLayout = LuaLayout {
        ref_size: 8,
        gcstr_len_off: 0x10,
...
mod tests {
    ...
    /// Build a minimal `GCtab` (BITSQUID_LUAJIT layout) with a 3-element array part
    ...
    fn reads_array_and_hash_parts_of_a_hand_built_table() {
        let layout = LuaLayout::BITSQUID_LUAJIT;
        let tab = Va(0x1000);
