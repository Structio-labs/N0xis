...
pub struct DecodedInsn {
    ...
    /// On a return, the number of argument bytes the **callee** pops —
    /// `ret 8` is `Some(8)`. `None` on everything else, and on a plain `ret`.
    ///
    /// This is the cheapest arity signal any format offers and it was being
    /// discarded: `ret`, `ret imm16`, `retf` and `retf imm16` all lifted to the
    /// same `Return`. On a 32-bit PE, where arguments are passed on the stack
    /// and no register-based recovery can see them, **2 361 of 3 130 returns**
    /// carry one — and without it every one of those functions rendered as
    /// `f(void)`, which is not "unknown", it is a statement that there are no
    /// parameters.
    #[serde(default, skip_serializing_if = "Option::is_none")]
