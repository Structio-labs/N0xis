...
/// The `Unknown` marker a call leaves on every register its ABI says the
/// callee may destroy. It states "this register has no value here" — not "the
/// lift failed", which is what every other `Unknown` means — and consumers
/// have to tell the two apart: the renderer hides it, the emulator leaves the
/// variable undefined so a read of it reports the ABI fact at the read.
///
/// One string, one definition. It was spelled out at four sites, which is the
/// shape every drift in this project has taken.
pub const CALL_CLOBBER: &str = "call-clobbered";

/// The reserved variable an **indirect jump** computes its destination into.
///
/// A direct jump is structural — the CFG carries the edge and the address is
/// in the instruction. An indirect one is not: `jmp *(%rdx,%rax,8)` computes an
/// address that exists nowhere else, and dropping it left the IR unable to say
/// where a switch goes. The CFG's resolved cases are a *claim* about that
/// address; this is the address itself, so the two can be compared.
///
/// Never a valid register name, so it shares the `Var` namespace safely — the
/// same arrangement `FLAGS_VAR` uses.
pub const JUMP_TARGET_VAR: &str = "__jump";
...
pub enum CmpKind {
    Cmp,
    Test,
    /// Flags left by an **arithmetic** op that *keeps* its result in `lhs`
    /// (`dec ecx`, `sub rax,rbx`, `add`, `neg`). Only the zero flag —
    ...
    /// is the constant `0`.
    Result,
    /// Flags left by a **logical** op that keeps its result in `lhs`
    /// (`and`/`or`/`xor`). A logical op clears OF and CF to 0, so *every* signed
    /// and unsigned condition is a pure function of the stored result's sign and
    /// zero-ness — `branch_condition` reconstructs the whole `jcc` family (not
    /// just `je`/`jne`), exactly as for `Test`. `rhs` is the constant `0`.
    LogicalResult,
}
