...
pub struct SwitchDispatch {
    ...
    /// Full-width name of the register holding the **table base**, when the
    /// dispatch reads the table through one.
    ///
    /// It exists because a compiler hoists the `lea table(%rip), %rsi` out of a
    /// loop whenever the dispatch is inside one — which is most of the time —
    /// and a scan that only looks inside the dispatching block then finds no
    /// table at all. Knowing *which register* holds it is what lets the search
    /// continue backwards precisely, instead of widening the scan and risking
    /// an unrelated `lea`.
