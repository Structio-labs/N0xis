...
//!   vector/crypto code.
//! - [`Arch::lift`] and [`Arch::branch_condition`] cover the **base integer
//!   families a compiler emits for scalar code** and nothing else — see
//!   [`crate::arm64_lift`], which owns the lowering and lists exactly what is
//!   in and out. Loads and stores are the largest thing still out; everything
//!   unnamed there keeps the trait's `Unlifted` default, so the gap is
...
static AAPCS64_CC: CallConv = CallConv {
    ...
    // The lift models the integer register file only, so there is no modelled
    // float return register and nothing to invalidate across a call. `None`
    // states that about the model; it is not a claim about the ABI, which does
    // return in `v0`/`s0` and clobbers `v0`-`v7`+`v16`-`v31`.
    ret_float: None,
    volatile_float: &[],
    ret_float_second: None,
    // Same as `ret_float`: a statement about this model, not about the ABI —
    // which does pass floating-point arguments in `v0`-`v7`.
    float_args: &[],
...
pub(crate) fn bitrange(bits: u32, hi: u32, lo: u32) -> u32 {
    let width = hi - lo + 1;
...
pub(crate) fn sign_extend(value: u32, width: u32) -> i64 {
    let shift = 32 - width;
...
///
/// `w0` and `x0` are one physical register, and recording them as two names is
/// the defect this project has already paid for once on x86 — where `reg_name`
/// answers with `full_register()` for exactly this reason. On AArch64 the name
/// followed the `sf` bit instead, so a def-use record said `w0` while a query
/// said `x0`, and `ir slice --reg x0` reported `node_count: 0` on `emu_add32`,
/// whose only instruction is `add w0, w0, w1`. That is not a smaller answer, it
/// is a statement that nothing computes the function's return value. C's `int`
/// is 32 bits, so W-register arithmetic is most of what a compiler emits.
///
/// The *disassembly text* still says `w0` — that is what the instruction reads,
/// and it is right. Only the records are canonical, exactly as on x86 where the
/// text says `eax` and the record says `rax`. [`Arch::normalize_reg`] folds a
/// query the same way, so asking about either spelling finds the register.
pub(crate) fn gpr_name(bits: u32, n: u32, want_zr: bool) -> String {
    let _ = bits;
    get_int_reg_name(true, n as u8, want_zr).to_string()
}
...
fn build_insn(bits: u32, va: Va) -> DecodedInsn {
    match decoder::decode(bits) {
        Some(op) => {
            ...
            // **The mnemonic is the first word of the instruction, not the name
            // of its encoding family.** The decoder's definition calls every
            // conditional branch `b.c`, while the rendering resolves the
            // condition — so one decoded instruction carried `mnemonic: "b.c"`
            // beside `text: "b.ne 0x44"`, two answers in two fields of the same
            // object. Compared with an independent disassembler over a real
            // AArch64 object, that was the only disagreement out of 40 that was
            // not a legitimate alias.
            //
            // Taking it from the text makes the two agree by construction. The
            // *class* still comes from the definition — `classify` above — which
            // is a question about the encoding and rightly asked of it.
            let mnemonic = text.split_whitespace().next().unwrap_or(def.mnemonic).to_string();
            DecodedInsn {
                va,
                len: 4,
...
impl Arch for Arm64 {
    ...
    fn lift(&self, insn: &DecodedInsn, abi: &str) -> Vec<crate::MicroStmt> {
        crate::arm64_lift::lift(self, insn, abi)
    }

    fn lift_tail_call(&self, insn: &DecodedInsn, abi: &str) -> Vec<crate::MicroStmt> {
        crate::arm64_lift::lift_tail_call(self, insn, abi)
    }

    /// AArch64 spells its condition codes differently from the vocabulary
    /// [`crate::flags`] speaks, so this translates first — and that translation
    /// is also where the two architectures' opposite carry-flag polarity is
    /// reconciled. See `arm64_lift::x86_spelling`.
    fn branch_condition(&self, mnemonic: &str, flags_value: &crate::MicroExpr) -> crate::MicroExpr {
        crate::arm64_lift::branch_condition(mnemonic, flags_value)
    }

    /// AArch64 instructions are 4 bytes and entries are at least that aligned.
    ...
    /// Fold a register spelling to its canonical 64-bit form, so one physical
    /// register has one identity however it was written.
    ///
    /// `w3` and `x3` are the same register; so are `s0`, `d0`, `q0` and `v0`,
    /// which is the same trap one register file over. `lr` and `fp` are the
    /// procedure-call names for `x30` and `x29`, and a person asking about
    /// `lr` means `x30`.
    fn normalize_reg(&self, reg: &str) -> String {
        ...
        match r.as_str() {
            "lr" => return "x30".into(),
            "fp" => return "x29".into(),
            "wzr" | "xzr" => return "xzr".into(),
            "wsp" | "sp" => return "sp".into(),
            _ => {}
        }
        let fold = |prefix: char, to: &str| -> Option<String> {
            let n: u32 = r.strip_prefix(prefix)?.parse().ok()?;
            (n <= 31).then(|| format!("{to}{n}"))
        };
        // The 32-bit view of a general-purpose register…
        if let Some(x) = fold('w', "x") {
            return x;
        }
        // …and the narrow views of a vector register, which are one register
        // under five names.
        for p in ['b', 'h', 's', 'd', 'q'] {
            if let Some(v) = fold(p, "v") {
                return v;
            }
        }
        r
...
mod tests {
    ...
    fn decodes_cbz_as_cond_jump_and_reads_the_tested_register() {
        ...
        // The record names the register **canonically**, at 64 bits, even
        // though the instruction reads its 32-bit view. `w0` and `x0` are one
        // register; two names for it is what made `ir slice --reg x0` answer
        // `node_count: 0` on a function whose only instruction writes it.
        assert_eq!(access.reads, vec!["x0".to_string()]);
    ...
    fn decodes_register_form_add_with_two_reads() {
        ...
        let access = arch.reg_access(&di);
        assert_eq!(access.writes, vec!["x0".to_string()]);
        assert_eq!(access.reads, vec!["x0".to_string(), "x0".to_string()]);
    }

    /// **One register, one name — at every width.**
    ///
    /// The same operation encoded at 32 and at 64 bits must produce the same
    /// def-use record, because `w0` and `x0` are the same sixty-four bits of
    /// silicon. Recording them separately is the defect this project has
    /// already paid for once on x86 (`xmm0` vs `zmm0`), and it was open here:
    /// `ir slice --reg x0` reported `node_count: 0` on a function whose only
    /// instruction is `add w0, w0, w1`, which is to say the tool denied that
    /// anything computes the function's return value. C's `int` is 32 bits, so
    /// that is most of what a compiler emits.
    ///
    /// This test is the one that was missing. The three above asserted the
    /// 32-bit spelling and so held the defect in place.
    #[test]
    fn the_same_operation_at_two_widths_names_one_register() {
        let arch = Arm64::new();
        let names = |bits: u32| {
            let di = arch.decode(&le(bits), Va(0x1000)).unwrap();
            let a = arch.reg_access(&di);
            (a.reads, a.writes)
        };
        // add x0, x1, x2   vs   add w0, w1, w2 — the same three registers.
        assert_eq!(names(0x8b020020), names(0x0b020020), "one register, one name");
        // And a query for either spelling folds to the same identity, so the
        // records and the question meet.
        for (asked, canonical) in
            [("w0", "x0"), ("x0", "x0"), ("wzr", "xzr"), ("lr", "x30"), ("d5", "v5"), ("q5", "v5")]
        {
            assert_eq!(arch.normalize_reg(asked), canonical, "normalize {asked}");
        }
    }
