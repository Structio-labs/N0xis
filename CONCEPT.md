...
6. **Never silently lose semantics.** Any instruction/pattern the analysis can't
   ...
   measurement, not an intention: `crates/n0xis-arch/examples/prefix_audit`
   reports, per decoder flag, how many instructions carry it and how many the
...
made **`Arm64`** possible (via disarm64: CFG / discover / xref / goto+structured
decompile) without rewriting analysis. Where it now stands, measured on a real
AArch64 library (2026-09-07): the **decoder and CFG are verified to x64's
standard** — 2 381 of 2 381 function extents exactly equal to the symbol table,
0 invariant violations across 46 757 CFG edges. SSA optimization and
flag-precise conditions remain **x64-only**, so `decomp pseudo` on ARM64 reports
`quality: 0.0` and falls back to `asm` nodes rather than guessing.

That verification is also what found the seam's own defect: nothing consulted
the *image's* declared machine, so an AArch64 file was decoded as x86-64 by
default — and the IR cache, keyed without the decoder, then served that answer
to a caller who had explicitly asked for arm64. An ISA is a fact the file
states; the seam now takes it from there.
