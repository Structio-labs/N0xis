...
> Status: **design / greenfield**. v0 is archived in [`archive/`](archive/).
> Companion docs: [`ROADMAP.md`](ROADMAP.md) · [`docs/CLI_COMMANDS_v0.md`](docs/CLI_COMMANDS_v0.md).
> Created 2026-07-08.
...
afterthought. N0xis fuses them, and does it **contract-first (CLI+MCP), GUI-never**.
...
- **Contract-first, GUI-never.** Every capability is a CLI verb + MCP tool that
  returns a versioned JSON artifact (`ok/data/meta`). A human reads it; an agent
  parses it. There is no hidden interactive state.
...
n0xis-core   (pure analysis — NO I/O, NO OS)
    ...
    depends only on the seams below (traits, never concrete impls):
        ├── n0xis-arch      trait Arch          (+ X64 impl, iced-x86)
...
| `n0xis-arch` | ISA abstraction (`trait Arch`) + `X64` impl (iced-x86): decode, lift-to-microIR, register model, calling conventions. | contracts, iced | OS, I/O, sources |
| `n0xis-sources` | Input adapters: `MemorySource` / `SymbolProvider` / `ModuleProvider` + `LiveProcess` (Win32), `StaticPe` (goblin), `Snapshot`. | contracts, windows-sys, goblin | analysis logic |
| `n0xis-core` | Pure analysis passes over `Arch` + source traits: CFG, IR, **SSA, propagation, DCE**, type inference, control structuring, pseudo-C render, xref, slice. **No I/O, no OS.** | contracts, arch (trait), sources (traits) | concrete adapters |
| `n0xis-project` | `.n0x/` analysis database: functions, names, types, comments, selections, patches, dumps. Versioned truth. | contracts | analysis logic |
...
Implementations: `LiveProcess(pid)`, `StaticPe(path)`, later `Snapshot` (cached
memory dump for reproducible offline runs) and `RemoteAgent` (analysis over SSH/
Tailscale to another machine — natural fit for the existing PC-to-PC setup).
...
`X64` is the only impl at first, but *all* x64/Win64 knowledge (arg regs, volatile
regs, ABI, flag semantics) lives here — never in the passes. This is the seam that
makes ARM64/other targets possible later without rewriting analysis.
...
|---|---|---|
| Primary interface | GUI; scripting bolted on | CLI + MCP contract, GUI-never |
...
unified live/static, reproducible.
...
- **Every output is a versioned schema** in `n0xis-contracts`; breaking a schema
  bumps its `vN`.
- **CLI surface from [`docs/CLI_COMMANDS_v0.md`](docs/CLI_COMMANDS_v0.md) is
  preserved**; new capabilities are additive.
...
- **Hooks / timers** — value-change watchpoints (HW breakpoints), periodic re-scan;
  a speedhack-style time hook is a later stretch.

Every one of these is a CLI verb + MCP tool returning a schema'd artifact — so an
agent can drive a full scan→filter→freeze loop headlessly.
...
fusion, snapshot-diff causal attribution, cross-version binary diffing, …) live in
the registry.
...
- **New v1 wire schemas:** `n0xis.*.vN`. The archived v0 schemas remain `n0x.*`
  (historical — see [`docs/CLI_COMMANDS_v0.md`](docs/CLI_COMMANDS_v0.md)).
- **Kept as-is for back-compat:** the project dir `.n0x/`, the `n0x.cmd` shim, and
  the `.n0xt` table extension (reads as "n0x table").
- **GitHub:** username `n0xis` is a dormant squatter; use an org variant
  (`n0xis-re` / `getn0xis`) or a personal repo named `n0xis`.
