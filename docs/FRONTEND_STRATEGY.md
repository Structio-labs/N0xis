...
| Symbols / Signatures | FLIRT (`sig gen`/`--flirt`) — the WARP interoperability layer is a separate plugin outside this repository, so this engine backs no `.warp` panel |
...
- **Analysis** — auto-analysis depth, signature databases (FLIRT `.npat`),
  demangler options, arch override, symbol sources order.
