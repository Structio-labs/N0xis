...
**Verification discipline (binding).** A `shipped` status says the code exists and
passes *its own* tests. It does **not** by itself say the *scope* claim ("beats
other X here") was fact-checked, nor that the feature was verified against a live
target. Where those differ, the entry says so explicitly: **`self-tested`** (unit
tests, no live target), **`live-verified`** (run against a real process/binary), and
**`scope claim unaudited`** (the "no other does this" line is a design
judgement, not a review bench). We keep "implemented and self-tested" strictly
...
| Pointer-path / AOB | ✗ / basic | ✅ | ✅ **typed** paths (planned) |
| Find-what-writes | ✅ (debugger) | ✅ | ✅ **+ auto-decompile** that code |
| Cross-process caller chain on a HW hit | in-process only | ✗ | ✅ **.pdata/.xdata unwind from outside** |
| Persistent table | project DB | `.CT` | `.n0xt` + code/type/provenance links |
| Survive game updates | manual re-RE | ❌ tables break | ~ **anchors + `sig validate`** (self-heal planned) |
| Localize an unknown value | manual scan | ✅ scan loop | ✅ **"the change is the signal"** as a primitive |
...
### KF-1 — Provenance-Driven Memory Intelligence  ·  *principal*  ·  status: `shipped` (Phase 4c, live-verified for value→meaning)
...
- **Feature:** (a) *value → meaning*: scan → find-what-accesses (HW watchpoint) →
  ...
  watchpoint × decompiler fusion is the principal and is verified live. (b) *intent →
...
- **What exists elsewhere:** localization in CE is a manual scan-and-narrow loop the operator
  drives by eye; other tools have no concept of "toggle one thing and keep what moved".
  Nobody ships the *method* itself — concept-grep, transition-diff, signature
  discipline, injection-flag probing, constant recognition — as first-class,
  agent-drivable commands.
...
- **Why it wins:** these are **method-as-tooling** — the difference between a human
...
- **Status detail:** 6/7 named commands **DONE and merged to `main`** (commit
  ...
  option** (⬜). *Scope claim unaudited* — the techniques are verified working; the
  "no other ships the method itself" claim is a design judgement, not a review
  bench. **Roadmap:** Phase 8.

### KF-8 — Position-by-Region UI Localization  ·  status: `building` (working-tree; implemented + self-tested, **pending live validation**)
- **What exists elsewhere:** to go from "this widget on screen" to "the memory that describes it",
  every existing path hooks a graphics API, captures frames, or reads pixels. No memory
  tool hit-tests the *target's own retained UI geometry*.
- **Gap:** nothing maps a **screen rectangle → the addresses of the scene-graph nodes
...
- **Feature:** [`ui locate --rect`](CLI_COMMANDS.md) enumerates live UI elements whose
  ...
  *relations between fields* (e.g. an AABB whose min ≤ max in each axis, finite,
  ...
  MCP (`ui_locate`).
- **Why it wins:** position-by-region hit-testing of a program's *own* scene graph, from
  a separate process, with no render-path instrumentation — an approach no other
  memory or RE tool takes.
...
- **What exists elsewhere:** other tools decode native code but treat engine bundle formats and
  script VMs as opaque blobs; CE has no concept of either. Reading a *running* LuaJIT
  VM's objects is bespoke, per-project work everywhere.
- **Gap:** no single toolkit does **static engine-asset extraction + offline bytecode
...
- **Feature:** a two-sided engine layer, all `n0xis.*.v1`-tagged:
  ...
  - **Live / in-VM:** [`lua strings`](CLI_COMMANDS.md) finds live `GCstr` objects by
    ...
    at once.
- **Why it wins:** static asset RE and *live* script-VM introspection under the same
  agent-native surface, with the live side needing no debugger attach — a combination no
  other offers. The live GCstr/GCtab introspection (`lua strings`/`lua table`) is
  ...
  (commits `4cc5f4e`, `d6580f2`). *Scope claim unaudited* as a scope claim.
- **Boundary note:** `n0xis-bitsquid`, `n0xis-lua`, and `n0xis-luajit` are **not**
...
- **What exists elsewhere:** debuggers unwind the stack **in-process** (they *are* the process's
  debugger with full context and often symbols). Turning a raw HW-watchpoint hit in
  *another* process into a real, named caller chain — from nothing but that process's
  own `.pdata`/`.xdata` — is not something CE or the typical scanner does at all.
- **Gap:** on a find-what-writes hit you get an instruction address, not "who called the
...
- **Feature:** a **pure cross-process x64 stack unwinder** driven by the target's PE
  ...
  boundaries.
- **Why it wins:** it turns the principal provenance hit (KF-1) from "an address" into "a
  named call stack", cross-process, with no in-process agent and no symbols required —
  precisely the semantic layer scanner-class tools lack.
...
- **N0xHUD — a companion window over the same engine.** A third frontend
  (`n0xis-hud`) that is a plain always-on-top `eframe`/`egui` window (the a separate always-on-top window
  model), **not** an in-game overlay and **not** framed as a trainer: it drives the same
  ...
  scope primitive; the interact-combo solver is the part most worth eventually
...
(What exists elsewhere / Gap / Feature / Why it wins / Roadmap), plus a **What exists now** line
when the shipped subset differs from the full vision, and an explicit **verification
state** (`self-tested` / `live-verified` / `scope claim unaudited`, and
...
*why* each edge exists. Never let a `shipped` flag silently imply the scope claim
was benched or that the feature was validated live; if it wasn't, say so.
